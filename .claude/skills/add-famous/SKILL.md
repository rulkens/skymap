...
along. `famous.bin` / `famous_meta.json` are build artefacts (and legacy-tracked,
so they may show as modified) — they reach prod via R2, not git, so don't fret
over their git state here.
