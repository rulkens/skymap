/**
 * pausableDwell — the interruptible dwell timer of a guided-tour beat. After the
 * establishing fly lands, the beat holds on its subject for `beat.dwellSec`
 * while gentle ambient drift plays; the user can advance, step back, or
 ...
 * running→paused flip subtracts the elapsed slice from `remaining`, so resume
 ...
 * loop runs only as long as the user keeps toggling pause.
 *
...
/**
 * Hold on the beat's subject until the user advances / steps back / the timer
 ...
 */
export function* pausableDwell(beat: BeatData): Generator<unknown, BeatOutcome> {
  ...
  while (true) {
    ...
    const ev = yield* race({
      ...(paused ? {} : { timeout: delay(remaining) }),
    ...
    if (ev.next || ev.prev || ev.timeout) {
      if (paused) yield* put(setPaused(false));
      return ev.prev ? 'prev' : 'next';
    }

    // ev.toggle — flip pause state. Only a running→paused flip consumes time.
