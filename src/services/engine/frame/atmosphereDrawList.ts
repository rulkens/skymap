 ...
 * ONE entry passes: Earth is the only body with an atmosphere row today, and even
 * once Mars/Venus/Titan arrive the near-field cull keeps all-but-one culled at any
 * real camera pose (two atmosphere bodies are never a pixel across at once). The
 * list shape costs nothing and lets the consumers iterate without a special case.
 */
