 ...
 *
 * `state.gpu.pointRenderer`, `state.gpu.renderTargets`, and `state.subsystems.thumbnails`
 ...
 * `ctx.pointRenderer.draw(...)` directly, no `!` needed.
 *
...
export function deriveFrameContext(
  ...
  const pointRenderer = state.gpu.pointRenderer;
