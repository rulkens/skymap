/**
 * sceneOccluderSpheres — resolve THE per-frame set of bodies that can hide an
 * orbit trail, packed eye-relative in km for the conic fragment's segment test.
 *
 * A body occludes iff it is presented as an OPAQUE SPHERE this frame, so the
 * set is read off the partitions the `foreground:0` sphere layers themselves
 * consume, bound exactly as those binders bind them (`ctx.drawCamPos`,
 * `ctx.canvasSize.height`, `ctx.fovYRad`). "Drawn" would be the wrong fact: a
 * 1–3 px planet is drawn, as an additive glint, and occludes nothing. Radii
...
export function sceneOccluderSpheres(
  ...
  return {
    count: selectOccluderSpheresKm({ occluders, camPosMpc: ctx.drawCamPos }, spheresKm),
    spheresKm,
  };
}
