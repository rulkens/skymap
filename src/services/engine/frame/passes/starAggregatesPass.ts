 ...
 * walks once in `runFrame`, which sets `computeStarCut`'s result on
 * `state.gpu.starCatalogRenderer` before either draws, so this layer and
 * `starCatalogPass` both read that one cut rather than walking again. This
 * layer records ONLY the aggregate sub-stream, via the shared `drawStarStream`
 * helper with `stream: 'aggregate'` — the `fsLinear` pipeline into the offscreen.
 *
