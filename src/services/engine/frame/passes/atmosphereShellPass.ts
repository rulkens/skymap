 ...
 * per body per frame or the in-scatter doubles. The frame program expands this to one step per
 * body-m row, so `enabled`/`draw` runs once PER BODY on `view.slab.frame.bodyId`. Non-pickable
 * (a translucent halo has no clickable silhouette), so no `drawPick`. Argued elsewhere: bake↔draw
 * equality in `atmosphereDrawList`, the shell in `atmosphereShellRenderer` + `shell/fragment.wesl`,
 * this row's order in `frameOrder.ts`, the uniform record in `atmosphereShellUniforms`.
 */
...
export const atmosphereShellPass: ContentPass = {
  ...
  enabled(state, ctx, view) {
    ...
    return atmosphereDrawList(state, ctx).some(
      (entry) => entry.body.id === bodyId && entry.inside === false,
    );
  },
