 ...
 * epoch to `ctx.snapshot.simDays` — the frame's sim-clock instant, derived once by
 ...
 * stamping a new `ctx.snapshot.simDays`, with no consumer touched.
 *
 * ### `state` unread
 *
 ...
 * unread: the epoch this needs lives on `ctx.snapshot.simDays`, and the element table is
 * a compile-time import, so nothing on `PassState` is consulted. The
...
export function sceneBodyStates(state: PassState, ctx: FrameView): ReadonlyMap<string, BodyState> {
  return deriveBodyStates(ctx.snapshot.simDays);
}
