 ...
 * (no cast) and delegates to that arm's commit helper. Replacing the old
 * `isStructure(target) ? … : …` predicate with a `Record<FocusableTargetType,
 * …>` table follows the simplicity convention's table-dispatch rule (item 7):
 * adding a new focusable kind (e.g. the Milky Way) is one new row here, not an
 * edit to a growing predicate chain that every dispatch site would have to
 * keep in sync.
 *
