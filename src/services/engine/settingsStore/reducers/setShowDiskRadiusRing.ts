/**
 * setShowDiskRadiusRing — pure reducer for the disk-radius debug-ring toggle.
 *
 ...
 * input in place (`state.debug.showDiskRadiusRing = show`) would defeat both,
 * hence the spread.
 *
...
export function setShowDiskRadiusRing(
  state: EngineSettingsState,
  show: boolean,
): EngineSettingsState {
  return { ...state, debug: { ...state.debug, showDiskRadiusRing: show } };
}
