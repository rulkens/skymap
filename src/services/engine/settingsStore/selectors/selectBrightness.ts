/**
 * selectBrightness — pure projection of the shared survey billboard brightness.
 *
 ...
 * changes; the engine reads the same field off `state.settings.surveys.brightness`.
...
export function selectBrightness(state: EngineSettingsState): number {
  return state.surveys.brightness;
}
