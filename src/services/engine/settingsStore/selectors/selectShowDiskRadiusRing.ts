/**
 * selectShowDiskRadiusRing — pure projection of the disk-radius debug-ring toggle.
 *
 * The read seam shared by both sides: React subscribes with
 * `useSettingsStore(handleRef, selectShowDiskRadiusRing, …)` and re-renders only
 * when the value changes; the engine reads the same field off
 * `state.settings.debug.showDiskRadiusRing` each frame. The projection returns a
...
export function selectShowDiskRadiusRing(state: EngineSettingsState): boolean {
  return state.debug.showDiskRadiusRing;
}
