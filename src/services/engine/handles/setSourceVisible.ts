...
// on/off.  It writes that flag by dispatching the `setGalaxyCatalogVisible`
// slice action rather than mutating the held object in place:
// `store.dispatch(...)` is what NOTIFIES React's
// `useAppSelector(selectVisibleSourceMask)` subscriber so the panel checkbox
...
// row's `galaxyCatalog` handle. ORDERING MATTERS: the dispatch MUST precede
...
// `enabled` bits the slice action writes. One source of truth, projected on
...
import { setGalaxyCatalogVisible as setGalaxyCatalogVisibleAction } from '../../../state/settings/settingsSlice';
...
export function setSourceVisible(
  ...
  // Single source of truth: flip the galaxy catalog's enabled flag by dispatching
  // the slice action so the write notifies React's selector subscriber.
  store.dispatch(setGalaxyCatalogVisibleAction({ id, enabled: visible }));
