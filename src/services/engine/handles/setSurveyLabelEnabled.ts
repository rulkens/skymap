...
// the engine-owned store (the `setSurveyLabelEnabledAction` copy-on-write
// reducer) rather than mutating the held object in place: the store write is
// what NOTIFIES React's `useSettingsStore(selectSurveyItems)` subscriber so the
...
import { setSurveyLabelEnabledAction } from '../settingsStore/actions/setSurveyLabelEnabledAction';
...
export function setSurveyLabelEnabled(
  ...
  // Single source of truth: flip the survey's labelEnabled flag THROUGH the
  // store so the copy-on-write write notifies React's selector subscriber.
  setSurveyLabelEnabledAction(store, survey, enabled);
  ...
  // No echo: React reads the record via `selectSurveyItems` + a projection.
}
