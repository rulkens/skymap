...
//
//   1. Write the mode intent THROUGH the store (`setBiasModeAction`'s
//      copy-on-write reducer). The shader branches on the integer mode (0 =
//      none, 1 = volume-limited, …), so flipping it takes effect next frame
//      with no pipeline rebuild. React reads via `selectBiasMode`, so no echo
//      is wired.
...
export function setBiasMode(state: ApplyIntentState, store: SettingsStore, mode: BiasMode): void {
  setBiasModeAction(store, mode);
