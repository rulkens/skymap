 ...
 * the click target tracks the sprite exactly. Because `computeStarCut` only
 * reaches this stream when the source's crossfade is > 0, `opacity` here is
 * `crossfade × nodeFade` and is zero iff the node fade is zero.
 *
 * ### No re-traversal
 *
 * This consumes the SAME `PreparedStarCut` the visual pass built this frame
 * (memoised on `ctx`), so it never re-walks the octree or re-advances the LOD
 * fades — it is a linear filter over the already-partitioned leaf arrays. A
 * source whose leaf stream is entirely opacity-0 (or empty) is omitted from the
 * result, so the pick draw issues no work for it.
 */
