 ...
 *
 * A lit sphere binds one per-draw uniform buffer carrying its MVP, the sun
 * direction already rotated into the body's local frame, and the ambient
 * floor. This is the single source of truth for that byte layout: the Earth
 * layer (and any future dedicated lit-sphere renderer) packs through here so
 * the CPU write can never drift from the WGSL struct — a drift the GPU would
 * not report, it would just render the wrong frame (on iOS, drop it silently).
 *
 ...
 *   f32 19     (byte 76..79): ambient (folds into the vec3's vec4 tail)
 *
 ...
 * @param ambient      The ambient floor packed into the tail; the shared value
 *                     lives in `lib/bodyLighting.wesl` (`AMBIENT`).
 */

import type { Vec3 } from '../../@types/math/Vec3';

/** f32 count of `LitBodyUniforms` — 16 mvp + 3 sunDirLocal + 1 ambient. */
...
export function packLitBodyUniforms(
  ...
  out[19] = ambient; // byte 76 — the vec4 tail after the vec3
  return out;
}
