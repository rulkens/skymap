 ...
 *  format: the bake and the CPU decoder land on the same float per code. Lives
 *  in `src/` and is imported BY `tools/` (never the reverse, per
 *  `utils/random/gaussian.ts`) because §5.3 allows exactly one such mapping. */
