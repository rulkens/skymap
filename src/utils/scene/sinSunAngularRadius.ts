 ...
 * the sphere-light width `pbrDirectSphere` takes. The Mpc → m conversion lives
 * here, as in `sunVisibleFraction`, so the frame pass never spells the seam.
 */
export function sinSunAngularRadius(
  ...
  return Math.sin(angularRadiusRad(sunRadiusM, distanceM));
}
