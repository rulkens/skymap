 ...
 *
 * The store combines six slices — `settings`, `ui`, `tier`, `selection`,
 * `selectionRows`, and `dataStatus` — via `combineReducers`. This
 ...
 *
 * The selection fold adds three sibling routes: `selection` holds the raw
 * identity-Intent refs (hover/select/focus); `selectionRows` is the
 * saga-owned derived display cache that InfoCard reads; `dataStatus` is the
 * serializable catalog-readiness descriptor that lets sagas re-resolve refs
 * after a cloud commit without touching React.
 *
