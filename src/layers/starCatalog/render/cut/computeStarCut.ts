 ...
 * nodes) IS its output. `null` when the master toggle is off.
 *
...
export function computeStarCut(
  ...
  const glowOverlap = settings.glowOverlap;
  const aggregateIntensityCap = settings.aggregateIntensityCap;
  ...
  for (const { source, catalog, entry, crossfade: sourceCrossfade } of starSourcesInBand(
    runtime,
    settings,
    camDistPc,
  )) {
