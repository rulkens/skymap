 ...
 * the ecliptic pole, so `planeFrameFromPole` reproduces `ECLIPTIC_FRAME`
 * exactly (it equals `planeFrameFromPole(270, 66.56)` to floating-point noise);
 * `ECLIPTIC_FRAME` is written out directly from the obliquity so the 23.44° has
 * a single, legible source.
 *
 ...
 * right-handed basis. Module-local: a fixed authored derivation lives beside the
 * frames it builds, not in `src/utils/`.
 */
