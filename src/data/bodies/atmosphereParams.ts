...
export const ATMOSPHERE_PARAMS: Readonly<Record<string, AtmosphereParams>> = {
  ...
  pluto: {
    // The blue haze ring — the only row whose Mie term is per-channel. New Horizons
    // measured peak I/F ~0.3 in MVIC red against 0.7-0.8 in MVIC blue at the SAME
    // 167° phase, so geometry cancels and that factor 2.5 IS the tholin-aggregate
    // cross-section's colour. Visible only BACKLIT: below ~100° phase the haze is
    // 1-3% of the sunlit disc, so eyeball it at phase >160°, ~7 Pluto radii out.
    // Every derivation, paper and known lie of this model against the real
    // atmosphere (~20 discrete layers, a backscatter lobe, an altitude-dependent
    // phase function — none representable here) is in
    // `.superpowers/sdd/2026-08-16-add-pluto-charon/pluto-atmosphere-research.md`.
    planetRadiusKm: seededPlanet('pluto').radiusKm,
    // [D] LORRI's haze noise floor sits ~260 km up (Cheng+17); 250 km is also 5 haze
    // scale heights. Thickest shell in the table relative to its body (ratio 1.21) —
    // correct, Pluto's atmosphere really is enormous for its size.
    atmosphereTopKm: seededPlanet('pluto').radiusKm + 250,
    // [D] Earth's Rayleigh scaled by surface number density (11 µbar / 40 K =
    // 7.82e-5 of sea level). ~300x under the haze: honest, and invisible.
    rayleighScatter: [4.5e-7, 1.06e-6, 2.59e-6],
    // [M] Gas and haze share ONE measured falloff (Young+18: haze extinction ∝ N2
    // density over 26-100 km). The isothermal 40 K value, 19 km, is the WRONG
    // number for this profile — the atmosphere climbs past 110 K by 30 km.
    rayleighScaleHeightKm: 50,
    // [D] Vertical scattering optical depth 0.013 over a 50 km scale height
    // (Gladstone+16) = 2.72e-4 /km at LORRI's 607.6 nm pivot, spread over
    // 680/550/440 nm by the measured blue/red ratio above (λ^-3.44).
    mieScatter: [1.85e-4, 3.83e-4, 8.25e-4],
    // [D] Tholin k=0.018 at 607.6 nm through van de Hulst: single-scattering albedo
    // 0.966, so absorption is 3.5% of scattering — present, never visible.
    mieAbsorption: 9.6e-6,
    // [M] "typical brightness scale heights of ~50 km" (Gladstone+16).
    mieScaleHeightKm: 50,
    // [D] Henyey-Greenstein solved against the one measured point on the phase
    // function, P(165°)~5 (Gladstone+16): g=0.5 gives 4.95. Far below Earth's 0.8
    // because HG must hit the forward lobe's HEIGHT, not merely lean forward —
    // raising g without cutting `mieScatter` by the same factor triples the ring.
    miePhaseG: 0.5,
    // [M] No ozone, and no oxygen-bearing species to make any: the Alice
    // occultation's inventory is N2, CH4, C2H2, C2H4, C2H6 and haze (Young+18).
    ozoneAbsorption: [0, 0, 0],
    ozoneCenterKm: 0,
    ozoneWidthKm: 0,
    // [M-ish] The seed's mean, 0.49, is darker than the measured Bond albedo
    // 0.72 ± 0.07 (Buratti+17). Kept on the seed anyway — the multi-scatter bounce
    // it feeds is a ~1% term at τ_vert 0.04 — but the SEED is the thing to fix.
    groundAlbedo: seededPlanet('pluto').albedo,
    // [L] Double Earth's: the whole visible ring sits AT the terminator, and real
    // night-limb haze is only ~4x fainter than day-limb (Cheng+17), so a hard fade
    // would cut the ring in half. Too wide and the glow leaks into deep shadow.
    twilightSoftness: 0.1,
    // [L] 1.0 = the physical result, no band gain.
    twilightIntensity: 1.0,
    sunIrradiance: 1.0,
    // [L] Earth's calibrated value as the starting point. Two MEASURED anchors to
    // tune against: at phase ≳148° the ring must be at least as bright as the
    // sunlit crescent (Cheng+17), and at ~20° it must be barely detectable
    // (I/F ~0.003, Gladstone+16). Obvious at both means this is too high.
    exposure: 2.35,
  },
};
