/**
 * cosmicWeb — the MCPM filament view: two Polyphorm volumes over a dark field,
 * galaxies off so the density is the only thing drawn. Copy is hand-written
 * and user-owned; nothing generates this file.
 */
...
// the viewer. Its registry row is default-off (a panel toggle), so the view
// turns it on explicitly — built from the registry defaults rather than indexed
// out of the seed, which is `Partial` by type.
