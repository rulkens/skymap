 ...
 * state: an `id`/`label` for identity and UI and the body's physical `radiusKm`.
 ...
 *
 * `radiusKm` is left in kilometres, the body's native unit, and is resolved into
 * a draw-space sphere by `composeBodyMvp` at render time rather than being
 * pre-scaled into Mpc here — keeping the authored number legible and the unit
