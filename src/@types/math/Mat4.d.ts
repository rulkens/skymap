/**
 * Mat4 — flat, mutable, column-major 4×4 matrix tuple used for model /
 * view / projection matrices.
 *
 * Column-major matches:
 *
 *   - gl-matrix (every `mat4.*` operation reads / writes column-major);
 *   - WebGPU / WGSL (the spec stores `mat4x4<f32>` as columns of
 *     `vec4<f32>`);
 *   - GLSL (the historical convention from which both the above derive).
 *
 * Mutable by default — gl-matrix's `mat4` is a mutable `Float32Array`,
 * and many of our renderer-internal accumulator matrices are filled in
 * place.  Consumers that want read-only access wrap with
 * `Readonly<Mat4>` at the boundary.
 *
