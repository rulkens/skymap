 ...
 * structure store.
 *
 * This is the structure-store rename of `PoiGroupId`'s `staticAnchors` /
 * `clusterBulk` pair: `anchors` holds the hand-curated featured structures,
 * `bulk` holds the catalog-derived remainder.  `PoiGroupId`'s `famous`
 * group is dropped here — famous galaxies are galaxy data, not structures,
 * so they live outside the structure store entirely.
 *
