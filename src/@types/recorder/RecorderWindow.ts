 ...
 * `installRecorderHook` and its only in-repo reader is that installer's test
 * suite (the harness reads it through untyped `page.evaluate`). Both sides
 * cast through this one name so the shape can't drift between them.
 */
