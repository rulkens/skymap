 ...
 * sweeps past at a chosen framing and (optionally) the aim swings to watch the
 * subject go by. Absent = through-centre (the historical behaviour), so a groups
 * flythrough that sets no `passBy` is untouched.
 *
 * The three knobs are one cinematographic concept — "the flyby" — grouped so the
 ...
 *   - `glance` — 0..1, how hard the aim tracks the subject through closest
 *     approach (0 = the look keeps leading down the path — a near-miss; 1 =
 *     the aim swings to frame the subject, then releases forward). Omittable.
 */
