 ...
 * the `surface` datum) is a pure function of these primitives, computed by
 * the maker so the authored data stays close to its catalogue units.
 *
 ...
 * `temperatureToLinearRgb`, `radiusSolar` → the surface datum) at the boundary rather
...
export type FamousStarRow = {
  ...
  readonly radiusSolar: number; // → the surface datum
