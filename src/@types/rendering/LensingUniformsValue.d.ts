...
/**
 * LensingUniformsValue — CPU mirror of the 272-byte LensingUniforms block
 ...
 * shader's `count`-gated loop runs zero iterations, so `mode` /
 * `scaleRadiusMpc` become don't-cares.
 */
export type LensingUniformsValue = {
   ...
   * (Mpc) + Einstein angular radius (rad). Capped at `MAX_LENSES` by the
   * packer; empty when lensing is off or no cluster sits in front of the
   * camera. See `lib/lensing.wesl` for the deflection model.
   */
  ...
  /** NFW scale radius r_s in Mpc — sets where the deflection peaks (the ring radius). Ignored in SIS mode. */
  readonly scaleRadiusMpc: number;
};
