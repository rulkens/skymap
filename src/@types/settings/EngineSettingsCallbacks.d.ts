...
export type EngineSettingsCallbacks = Pick<
  ...
  // `milkyWay` + `debug` + `volumes` dropped: those clusters live in the
  // engine-owned store, so the hook subscribes to no surveys/tonemap/auto-rotate/
  // bias/thumbnails/milkyWay/debug echo (the thumbnail + milkyWay toggles have no
  // React consumer at all; the DebugPanel reads the debug toggles via
  // `useSettingsStore` selectors). The volumes cluster (master `enabled` +
  ...
  // load slots still fire that echo against the full `EngineCallbacks` type;
  // harmless with no subscriber.) (Camera EVENTS — focus / camera / scale — are
  // not settings; `useEngine` wires those, not this hook.)
  'filaments' | 'labels' | 'input'
>;
