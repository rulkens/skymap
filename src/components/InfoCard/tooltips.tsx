...
export const TIPS: Record<string, TipContent> = {
  distance: {
    ...
    body: (
      <>
        ...
        parsec (pc): one parsec is 3.26 light-years, and a light-year is how far light travels in a
        year, about 6 trillion miles. Galaxies run to millions of parsecs, so you'll see megaparsecs
        (Mpc). We show light-years alongside because that's the unit most people picture; the slash
        just means "same distance, said two ways."
      </>
    ),
  },
  hubbleVelocity: {
    ...
    body: (
      <>
        ...
        <em>exceeds light</em>. That's allowed, because space is stretching; nothing is racing{' '}
        <em>through</em> it.
      </>
    ),
  },
  lookback: {
    ...
    body: (
      <>
        ...
        real galaxy has kept aging, reddening, and drifting farther off in the meantime. To look out
        into space is to look back in time.
      </>
    ),
  },
  earthEra: {
  ...
  ra: {
    ...
    body: (
      <>
        Where the galaxy sits east-to-west on the sky, the sky's version of longitude. Just as every
        place on Earth has a longitude, every star and galaxy has a right ascension, so anyone can
        aim a telescope at the same spot. It's counted in hours, minutes and seconds (0h to 24h)
  ...
  dec: {
    ...
    body: (
      <>
        Where the galaxy sits north-to-south on the sky, the sky's version of latitude. It runs from
        +90° at the north celestial pole (straight above Earth's North Pole), through 0° at the
  ...
  colour: {
    ...
    body: (
      <>
        ...
        a bluer one (e.g. <code>g − r</code>), the same idea as judging a heated iron bar by whether
        it glows red, orange or white. Hot, young, star-forming galaxies lean blue (small or
        negative values); old galaxies full of cool red stars lean red (large values). This one
  ...
  orientation: {
    ...
    body: (
      <>
        ...
        outline looks. <code>1.0</code> is a perfect circle (face-on, like looking down at a plate),{' '}
        <code>0.2</code> is a thin sliver (edge-on, the plate seen from the side).{' '}
  ...
  memberCount: {
    ...
    body: (
      <>
        How many catalogued galaxies currently fall inside this structure's sphere, given the tiers
  ...
  colourBpRp: {
    ...
    body: (
      <>
        A star's colour is a direct readout of its surface temperature. BP−RP is its brightness
        through a <strong>B</strong>lue filter minus its brightness through a <strong>R</strong>ed
        one (BP and RP are the Gaia satellite's two colour channels). A hot star floods out more
        blue light, giving a small or negative value; a cool star glows redder, giving a large one,
        the same physics as an iron bar heating from glowing red to white-hot.
      </>
    ),
  },
  ...
  spectralType: {
    ...
    body: (
      <>
        A letter code for the star's surface temperature, running O, B, A, F, G, K, M from hottest
        to coolest. O stars are searing blue-white, M stars cool and red, and our Sun a middling G.
        The digit after the letter (0–9) fine-tunes it, and a trailing Roman numeral notes the
        star's size class. Generations of students memorise the order with "Oh Be A Fine Guy/Girl,
        Kiss Me."
      </>
    ),
  },
  stellarRadius: {
    ...
    body: (
      <>
        The star's size in solar radii (R☉), multiples of our Sun's radius, itself about 700,000 km.
        A value of 1 matches the Sun; a red supergiant like Betelgeuse tops 700, so vast it would
        swallow the orbits of the inner planets. Stars swell and shrink dramatically over their
        lives, so size alone doesn't reveal a star's mass or age.
      </>
    ),
  },
  stellarTemperature: {
  ...
  stellarMass: {
    ...
    body: (
      <>
        The star's mass in solar masses (M☉), multiples of our Sun's mass. Mass is a star's destiny:
        it sets how fiercely the star burns, how long it lives, and how it dies. Heavyweights race
        through their fuel in a few million years and end as supernovae; lightweights sip theirs and
        glow for trillions. A small difference in mass makes an enormous difference to a star's
        whole life story.
      </>
    ),
  },
  stellarLuminosity: {
    ...
    body: (
      <>
        The star's total light output, in solar luminosities (L☉), how many Suns' worth of energy it
        pours out in every direction each second. That's different from how bright it looks to us,
        which also depends on distance. A dim-looking star can be staggeringly luminous if it's
  ...
  stellarAge: {
    ...
    body: (
      <>
        ...
        bright stars are always young, since they can't live long, while small red dwarfs glimpsed
        today could outlast the present universe many times over.
      </>
    ),
  },
  variability: {
    ...
    body: (
      <>
        Some stars don't shine steadily. They pulse, swell, or get eclipsed by a companion, so their
        brightness rises and falls in a repeating cycle. This row names the kind of variation and
        the magnitude range it swings across. Pulsating variables like Cepheids are astronomy's
  ...
  bodyMass: {
    ...
    body: (
      <>
        How much matter this world holds. We show planets in Earth masses (M⊕), where Jupiter is 318
        of them, and tiny moons in kilograms. Nobody puts a planet on a scale: its mass is read from
  ...
  bodyDayLength: {
    ...
    body: (
      <>
        How long this world takes to spin once, which is its day. Earth turns in 24 hours, Jupiter
        in under 10, but Venus is so sluggish (and spinning <em>backwards</em>) that its day
        outlasts its year. Most large moons are "tidally locked", turning exactly once per orbit so
        they keep one face toward their planet forever, which is why we only ever see one side of
        our own Moon.
      </>
    ),
  },
  bodyYearLength: {
    ...
    body: (
      <>
        How long one full trip around takes: around the Sun for a planet, around its planet for a
        moon. Kepler's law is the rule: the farther out you orbit, the slower you go and the longer
        the lap. Mercury races round the Sun in 88 days while Neptune plods through a single orbit
        for 165 <em>years</em>, so nobody alive has watched it complete one since its discovery.
      </>
    ),
  },
  bodyDistance: {
  ...
  bodyMeanTemp: {
    ...
    body: (
      <>
        ...
        lead. Its thick carbon-dioxide blanket traps heat in a runaway greenhouse, while airless
        Mercury dumps its heat straight back to space. An atmosphere can matter more than sunlight.
      </>
    ),
  },
  bodyMoons: {
    ...
    body: (
      <>
        ...
        dozens, from world-sized moons down to captured chips of rock, and Saturn's confirmed tally
        now runs past 140. The count keeps climbing as sharper surveys pick out ever-fainter specks,
        so read it as "known so far", not a final number.
      </>
    ),
  },
  bodyAxialTilt: {
  ...
  morphology: {
    ...
    body: (
      <>
        ...
        The shape traces how a galaxy formed and whether it's still building new stars in its arms.
      </>
    ),
  },
  agnClass: {
