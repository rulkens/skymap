 ...
 * row (see `rankPaletteMatches`).  `place` carries an `EarthPlace` — a
 * search-only point, not a focusable ref — ranked the same way but resolving
 * to a `flyTo` action instead of a focus.
 */
