 ...
 * container dispatches on: `{ kind: 'focus', focusId }` for every catalog
 * kind, using the same id scheme the URL deep-link layer uses, or
 * `{ kind: 'flyTo', ... }` for a place row (no focusable ref, just a camera
 * command).
 *
 * Every focus pick routes through the ONE selection command, `requestFocus`,
...
const ACTION_FOR_ROW: Record<ScoredRow['kind'], (row: ScoredRow) => PaletteAction> = {
  ...
  place: (row) =>
    row.kind === 'place'
      ...
      : { kind: 'flyTo', lonDeg: 0, latDeg: 0, altKm: 0 },
};
