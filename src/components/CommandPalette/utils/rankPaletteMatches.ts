...
export function rankPaletteMatches(
  ...
  // Famous rows, scene bodies, exhibits and tours are one class of primary
  // named object: merge and sort together so an exact body match ("earth")
  // outranks a famous row that only matched "earth" in its description. The
  // sort is stable, so earlier arrays stay ahead on an exact score tie.
  const primaryScored = [...famousScored, ...bodyScored, ...exhibitScored, ...tourScored].sort(
    (a, b) => b.score - a.score,
  );
