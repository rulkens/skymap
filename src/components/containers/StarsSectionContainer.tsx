 ...
 * `selectStarCatalogs` (master gate + per-catalog items) and wraps two dispatch
 * calls in `useCallback`. The presentational `StarsSection` imports nothing from
 * `store/` or `state/`.
 *
 ...
 * map to `setStarCatalogVisible`. `setStarCatalogLabelEnabled` is intentionally
 * left unwired here: star labels live in the separate Labels section, mirroring
 * how the Galaxies section exposes no per-catalog label toggle.
 *
...
function StarsSectionContainer(): React.ReactElement {
  ...
  const { enabled, items } = useAppSelector(selectStarCatalogs);
