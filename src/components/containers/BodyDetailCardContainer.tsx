 ...
 *
 * Owns the one read the presentational `BodyDetailCard` cannot do itself: the
 ...
 * radius, fact sheet) from its props and stays pure.
 *
 * `memo` localizes the pub's few-Hz re-render to this leaf. The distance selector
 * reads a primitive, so a republished-but-unchanged distance is a
 * reference-equal read and never re-renders the card — the identity rows are
 * stable across pub ticks. Every other detail prop is forwarded straight through.
 */
