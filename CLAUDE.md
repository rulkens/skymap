...
The runtime `cloudLoader` requests `<source>-<tier>.bin` per source as the user switches tiers; the `dataUrl()` helper prefixes each path with `VITE_DATA_BASE_URL`. **`.env*` files are gitignored** — `VITE_DATA_BASE_URL` is set as a build-time variable in the Cloudflare Workers project's dashboard (Workers & Pages → `skymap` → Settings → Variables and Secrets), so it stays out of the repo. Dev runs without the variable set, where `dataUrl()` falls back to the empty string and Vite serves `public/data/*` at the relative `/data/` path. A complete R2 sync must include every variant the runtime might request: `sdss-medium.bin`, `sdss-large.bin`, `glade-small.bin`, `glade-medium.bin`, `glade-large.bin`, plus the tier-agnostic `2mrs.bin`, `famous.bin`, and `filaments.bin`. The `tools/syncR2.ts` ALLOW filter encodes that set.

> **Local prod-build override.** Drop a `.env.production` file at the repo root (gitignored) with `VITE_DATA_BASE_URL=https://<r2-host>` if you want `npm run build && npm run preview` to hit the live R2 instead of the relative path. Useful for end-to-end verification outside the dev server.
