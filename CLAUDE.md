...
- **Pass files declare only the pass**: a file in `src/services/engine/frame/passes/` exports its `ContentPass` and nothing else — helpers go to `src/utils/` or `src/services/engine/frame/`, constants to `src/data/`, one symbol per file. Enforced by `tests/services/engine/frame/passes/passFilePurity.test.ts`, whose allow-list of today's offenders ratchets down, never up.
