...
- **cssEntry** is `.design-sync/dist/styles-combined.css`, regenerated by
  **`bash .design-sync/make-styles.sh`**: a Google-Fonts `@import` (Cormorant
  Garamond) + `src/styles/global.css` (the `:root` design tokens) + the
  Vite-extracted `dist/skymap.css` (component CSS) + two preview-only overrides
  (de-fix the fixed InfoCard; darken the harness's white preview page). Run it
...
bash .design-sync/make-styles.sh   # regenerate dist/styles-combined.css (cfg.cssEntry)
...
- The preview harness (`lib/emit.mjs`) hardcodes a white page + light cell
  chrome. `make-styles.sh` overrides it to dark, scoped to the harness classes
  (`.ds-grid` / `.ds-cell` / `[data-ds-fallback]`) so real designs are
  untouched. If a converter update renames those classes, update the override.
