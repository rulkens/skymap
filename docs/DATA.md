...
The bake emits `public/data/images/earth-tiles/v8/albedo/<z>/<x>/<y>.webp`: 512 px lossy WebP tiles whose alpha channel doubles as the land mask, plus two sidecars, `index.txt` (one path per line) and `manifest.json` (the flat band list every product's planner filters by `builtFrom`). The manifest is written after the tiles, so an interrupted bake leaves no pointer to half-baked data ([`buildEarthTiles.ts`](../tools/textures/buildEarthTiles.ts)).
