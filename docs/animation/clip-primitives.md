...
> _contract_ those tasks implement — treat this file as the source of truth for the
...
clip's **scene** changes (show a layer, fade it, change a setting) are _cues_ that
...
> should _look_ continuous hands off to another clip (e.g. a tour dwell uses a
...
frame. They change what's _drawn_, not where the camera _is_. All five are Layer 1
...
| `focus` | `focus(ref: SelectionRef \| null) → SceneEffect`          | Set selection focus to `ref` (`null` clears). On a **structure** ref this engages focus mode — the "dive in and isolate this cluster" lever, no `fade()` needed: (1) **non-member galaxies dim toward 0.08** — a per-galaxy, geometric isolation (members inside the structure's radius stay full-bright, with a soft smoothstep halo out to the apparent edge), and non-members also drop out of picking; (2) the surrounding **layers recede** (filaments, volumes, structure rings, labels). `null` releases both. Both ramp over the structure-focus blend; the dim is keyed to `selection.focus`, so a tour engages it exactly as a click does. |
...
- **`focusRecession`** — the structure-focus **layer** recession: under focus the
  ...
  recession targets. Galaxy points do **not** recede here. `focus(ref)` moves this.
- **`clipOpacity`** — the clip-owned transient channel. `fade()` moves _only_ this.
...
> `focus(ref)` / `selection.focus`, and is **galaxy-only** — not part of the layer
> product above. This is what makes `focus(ref)` "dim everything that isn't in this
...
- **Layer 2 — sagas (_Plan C_).** The guided tour: `BeatData`, `visitBeat` /
  ...
  click-to-advance. A beat _plays clips_ and `put`s plain Redux actions; it does not
