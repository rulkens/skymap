...
| ⬤   | issue                                                                                                                                                                                                                                                                                                                                                                                                                                                          | evidence                                          |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| 🟢  | `FADE_ROW` ↔ `VISIBILITY_ACTION_ROW` — **CLOSED, no longer a drift pair** (rung 7, #18 D2): the creator was lifted out of its closure into a required `writes` field, and `FADE_ROW` is derived from it in the same file; `watchFadesSaga` imports the result. One table, no drift pair                                                                                                                                                                        | `visibilityActionRow.ts:183-189`                  |
| ⚪  | **5 fade rows have no consumer** (union-totality artifacts) — corrected from 2 (rung 7, #18): add `scaleBar`; the full set is `proceduralDisks`, `texturedDisks`, `scaleBar`, `starCatalogLabel`, `bodyLabel`. **ACCEPTED, not open** — the three registration-only rows stay, because key-set equality with `VisibilityLayerKey` is what makes a new key a build failure until it gets a controller (#18 D13); the two label rows are rung 8's wire (#18 D12) | `fadeLayers.ts:108,115,153,166,174`               |
