...
| ⬤   | issue                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | evidence                                                         |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| 🟢  | `FADE_ROW` ↔ `VISIBILITY_ACTION_ROW` — **CLOSED, no longer a drift pair** (rung 7, #18 D2): the creator was lifted out of its closure into a required `writes` field, and `FADE_ROW` is derived from it in the same file; `watchFadesSaga` imports the result. One table, no drift pair. **Superseded by PR-B (layer composition)**: `FADE_ROW` and the `writes` field are gone; `watchFadesSaga` now re-syncs every fade row on any settings write (`syncFades()`), so there is no per-action lookup table left to drift                                                     | `visibilityActionRow.ts:131-141`                                 |
