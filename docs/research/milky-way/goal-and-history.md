...
- There are **no halo stars**. `SBb` → barred → `spiralLike`, whose halo count share is 0
  (`galaxyPopulationCountShares.ts:51`).
...
(`galaxyPopulationCountShares.ts:46`); at the preset's `bulgeSize: 0.45` that is **0.246**, and the
