...
- [ ] Add `starCaptionFadeAlpha.ts` — single function, file named for it. Didactic docblock: WHY a fade band and not a boolean gate (a boolean resolve gate pops the caption on/off as the camera crosses the threshold; a `smoothstep` band ramps the alpha so the caption grows in — the same continuous-envelope treatment the director gives famous labels, `labelDirectorSubsystem.ts:391,401`); WHY it shares the Task 4 star-resolve `thresholdPx` (one crossover point for the sphere AND its caption — do not fork the constant).
- [ ] Test `starCaptionFadeAlpha ramps a mid-band caption to a fractional alpha` — an `apparentSizePx` inside the band returns a value strictly between 0 and 1 (NOT 0/1), and the below-band / above-band ends return 0 / 1 (pins the band moved off a boolean, per the Defect-1 requirement).
