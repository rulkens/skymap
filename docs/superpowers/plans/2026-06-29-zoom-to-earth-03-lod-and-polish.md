...
- **VISUAL gates (NOT covered by automated tests — STOP and ask the user to confirm on the dev server; ALL require `?deepZoom` in the URL — without it `clampDistance` floors the wheel at 0.05 Mpc and the foreground bodies never grow past sub-pixel):**
  ...
  - **Star-label declutter (needs `?deepZoom`)** — the local-star captions no longer clobber into an unreadable pile from far out; each caption FADES in (no pop) as its star nears/resolves, overlapping captions de-collide, and there is no far-plane flicker at any distance; the star-point backdrop survives the deepest descent (~1 AU) without frustum-clipping out.
...
**Files:** `src/state/scene/watchFlyToEarthKeySaga.ts` (new), `src/store/rootSaga.ts` (modify — append the fork), `tests/state/scene/watchFlyToEarthKeySaga.test.ts` (new — mirror `watchTourKeyboardSaga` and `watchFocusTweenSaga` test harness style); plus (STOP-gate resolution, 2026-07-11) `src/store/types.ts` + `src/services/engine/engine.ts` — the saga could reach neither a selector nor a context Resource for Earth, so `SagaContext` gains a nullable `earthBody` getter registered in the `setSagaContext` block, mirroring `cameraRuntime` exactly.
...
- [x] Add `earthTextureFetcher.ts` — `Fetcher<ImageBitmap, void>`; `fetch(SCENE_EARTH.textureUrl, { signal })` + `createImageBitmap`, throw on `!res.ok`.
- [x] Add the `cameraDistanceMpc` surface to `DemandCtx` and populate it in `buildDemandCtx` from `state.cameraRuntime.lastPose.current.distance`.
- [x] Test (extend `buildDemandCtx.test.ts`): `buildDemandCtx surfaces the last produced pose's orbit distance as cameraDistanceMpc` — seed `state.cameraRuntime.lastPose.current.distance` and assert the ctx field equals it.
- [x] Add `earthTextureSlot.ts` — the factory + `EARTH_TEXTURE_MAX_DISTANCE_MPC`, per the contract + docblock above.
- [x] Test `createEarthTextureSlot sets the fetched bitmap on the earth renderer on ready` — mock `earthTextureFetcher` to resolve a stub `ImageBitmap`, stub `state.gpu.earthRenderer = { setTexture: vi.fn() }`, `slot.load(undefined as never)`, `await vi.waitFor(() => expect(slot.state().kind).toBe('ready'))`, assert `setTexture` called with the bitmap; pin construction purity (`slot.name === 'earthTexture'`, `state.assetSlots.earthTexture` undefined) — mirror `flowFieldSlot.test.ts:57-74`.
- [x] Test `createEarthTextureSlot commit is a no-op when the earth renderer is null` — `state.gpu.earthRenderer = null` (the destroy race); assert the slot still reaches `ready` and nothing throws (the IIFE's half-guarded `?.` bug, now covered).
- [x] Add the `'earthTexture'` member to `AssetKey` and the `earthTexture: AssetSlot<ImageBitmap, void> | null` field to `EngineAssetSlots` (docblock mirroring the `flow` null-until-`wireSlots` note); add the `ASSET_WIRING` row importing `createEarthTextureSlot` + `EARTH_TEXTURE_MAX_DISTANCE_MPC`.
- [x] Test (extend `assetWiring.test.ts`): `ASSET_WIRING includes a registry-built earthTexture row` (row present, `built` undefined, `req('medium')` is `undefined`) and `the earthTexture row demands the texture only within the descent threshold` — extend the test's `makeCtx` helper with a `cameraDistanceMpc` field (default a large distance / `Infinity`), then assert `demand(makeCtx({ cameraDistanceMpc: EARTH_TEXTURE_MAX_DISTANCE_MPC / 2 }))` is true and both `makeCtx({ cameraDistanceMpc: EARTH_TEXTURE_MAX_DISTANCE_MPC * 10 })` and `makeCtx({})` are false.
- [x] Delete the IIFE at `initGpu.ts:448-466`; verify the phase still constructs `earthRenderer` and stashes `phaseLocals`. (Optional: refresh the stale "fetched asynchronously by the engine (the NEXT task)" forward-reference in `EarthRenderer.d.ts:13` to name the slot.)
- [x] `npm test -- earthTextureSlot buildDemandCtx assetWiring` → green.
...
- [x] Raise the foreground caption size to the famous-comparable band in `sceneBodyLabels.ts`, sharing `FAMOUS_LABEL_STYLE`'s clamp constants rather than re-typing literals. Test `sceneBodyLabels sizes captions comparably to famous labels` — assert the emitted `minPixelSize`/`maxPixelSize` equal the shared famous clamps (pins the parity so a future famous-style change carries).
- [x] Add the leader-line emission + a rebased `foregroundMarkerLineRenderer` draw to `foregroundLabelsLayer` (or a sibling `foreground-marker-lines` row through the same `(swap, NEAR0)` step), consuming `labelLeaderLine`. Didactic docblock: WHY a second marker-line renderer instead of the director's (the NEAR0-vs-COSMO slab split — same reason the captions are a separate renderer); WHY the connectors are rebased (the AU-scale f64 seam the caption header already documents).
- [x] Test (extend `foregroundLabelsLayer.test.ts`): `foregroundLabelsLayer draws a leader line per caption` — a draw-spy on the foreground marker-line renderer asserts one connector per emitted caption, rebased (endpoints are camera-relative, not raw ~1-AU anchors). Mirror the existing layer draw-spy idiom.
- [x] `npm test -- sceneBodyLabels foregroundLabelsLayer` → green.
- [x] **VISUAL GATE (needs `?deepZoom`) — STOP and ask the user to confirm on the dev server:** star/planet/Earth captions read at a comparable size to nearby galaxy labels and hang off the body on a leader line, not painted over it. Commit after the user confirms.

## Task 10 — Star captions: fade-in resolve gate, declutter, and far-plane survival

The local star map's 24 captions (`SCENE_STARS`, `sceneBodies.ts:115-141`) all draw together, gated only by the layer-wide `ctx.cam.distance < SOLAR_SYSTEM_LABEL_MAX_DISTANCE_MPC` (1e-3) (`foregroundLabelsLayer.ts:99,122`) — there is NO per-star gate and NO declutter on the foreground caption path (the director's screen-space declutter at `labelDirectorSubsystem.ts:197-294` runs on the COSMO producer set, not this NEAR0 renderer). So from far out the captions pile into an unreadable clump. This task gives each caption a per-star **fade-in resolve gate**, de-collides the survivors, and — folding in two defects surfaced while flying the live foreground (2026-07-12, both confirmed against the code) — makes that gate a smooth alpha ramp rather than a boolean pop, and keeps the parsec-scale star field alive as the far plane collapses on a deep descent. The toggle, the declutter, and the fade band are ONE mechanism (a per-star visibility state feeding both the alpha and the cull), not three bolted-on gates.

**Reuse check (grounded):** the director's declutter is a greedy screen-space priority cull (project anchor → measured rect → accept if it clears accepted rects, `labelDirectorSubsystem.ts:197-294`), but it is welded to `ctx.vp` and the merged producer set — it cannot be called against the NEAR0 foreground captions without the slab tension of Task 9. So extract the minimal screen-space priority cull as a PURE helper the foreground path can call against the NEAR0 view. Per-star visibility reuses `apparentSizePx` + the Task 4 star-resolve threshold: a caption ramps in as its star nears/resolves — the same near-vs-far machinery the spheres use.

### Defect 1 — the resolve gate must FADE, not pop

A boolean "show when resolved" gate makes each caption snap on/off as the camera crosses its threshold on a zoom. The gate must instead drive a **fade band**: an alpha ramp over an apparent-size band straddling the star-resolve threshold, so a star mid-band gets a *fractional* alpha, not 0 or 1. The plumbing already exists end-to-end — `Label.fadeAlpha` (`Label.d.ts:89`) and `MarkerLine.fadeAlpha` (`MarkerLine.d.ts:22`) are premultiplied into every colour channel by the very shaders this layer draws through (`labels/vertex.wesl:45,91-96`; `markerLines/vertex.wesl:34,77`), and the label director already fades famous labels + their connectors by MULTIPLYING a continuous envelope into the producer's `fadeAlpha` (`labelDirectorSubsystem.ts:391,401`). The foreground caption path just needs to author that same field, which `sceneBodyLabels` today leaves at a constant alpha 1 (`sceneBodyLabels.ts:93` — no `fadeAlpha` set). Precedent for the band shape: `milkyWayApproachFadeAlpha.ts` is a `smoothstep`-band-to-alpha helper (reuse `src/utils/math/smoothstep.ts`), not a boolean cutoff.

### Defect 2 — far-plane survival for the parsec-scale star field

**Diagnosis (embedded so the implementer doesn't re-derive):** the NEAR0 slab's far plane is adaptive — `foregroundFrustum` (`foregroundFrustum.ts:58-62`) returns `far = max(camDistance × 100, FAR_MIN_MPC = 3e-11)`. Star anchors sit at parsec scale (Proxima ≈ 1.301 pc ≈ `1.3e-6` Mpc, `sceneBodies.ts`), so once orbit distance drops below ~`1.3e-8` Mpc (~2,700 AU) the `camDistance × 100` term falls below the nearest star anchor and the far plane sweeps *inside* the star field. Every star's caption AND its point sprite then frustum-clips out — the point sprite because `starPoints/vertex.wesl:70` writes `out.clip = center + offset` with NO depth clamp — each at its own per-anchor threshold, so they flicker one-by-one as the far plane moves per frame near the boundary. This is the SAME mechanism the `FAR_MIN_MPC` docblock already describes for the seeded orbit rings (`foregroundFrustum.ts:26-37`), recurring at star scale. A blanket far-floor raise is WRONG: the depth-tested bodies (Earth / star-spheres / planets) NEED the tight far plane for depth32float precision.

**Fix — scope it to the DEPTHLESS NEAR0 layers.** The star points, captions, and connectors are additive/OVER overlay draws with no depth test or write, so clamping their clip-space depth to just inside the far plane (`out.clip.z = min(out.clip.z, out.clip.w * (1.0 - EPS))`) keeps them from frustum-clipping without perturbing any depth comparison. The **star-points vertex stage MUST gain this clamp** (`starPoints/vertex.wesl` — the backdrop is always on screen, so it can never be handled by a fade). For the CAPTION side there is a decision to make, with a tradeoff — state which you took:

- **(A) Clamp the shared label + marker-line vertex stages too** (`labels/vertex.wesl:72`, `markerLines/vertex.wesl:69-72`). Both passes are depthless OVER on *every* path (COSMO director labels included), so the clamp is inert for depth and only suppresses far-clip — safe for the galaxy labels these shaders also serve. Cost: a shared-shader edit touching every label/line draw (verify visually that the COSMO labels are unchanged).
- **(B) Let the Defect-1 fade band gate the captions.** The clip bites only when zoomed in tight (small far plane), where an *unresolved* neighbour star's caption is already `fadeAlpha ≈ 0` — so if the band's fully-faded edge sits inside the clip threshold, the caption is never *visible* where the clip would bite and no shader clamp is needed for it. Cost: this COUPLES the fade band's far edge to `foregroundFrustum`'s far plane — encode the coupling (a shared constant, or cross-referencing comments) or it silently drifts into a flicker if either constant moves.

Only the star POINTS are non-negotiable; the caption clamp is A-xor-B. Do not do both blindly — the fade band (Defect 1) already removes the caption near the clip regime.

**Files:** `src/utils/scene/declutterByScreenSeparation.ts` (new — the pure cull) + `tests/utils/scene/declutterByScreenSeparation.test.ts` (new); `src/utils/scene/starCaptionFadeAlpha.ts` (new — the pure fade band) + `tests/utils/scene/starCaptionFadeAlpha.test.ts` (new); `src/services/engine/frame/passes/foregroundLabelsLayer.ts` (modify — per-star fade+declutter in `draw`, replacing the module-cached `BASE_LABELS` set with a per-frame set carrying `fadeAlpha`) + `tests/services/engine/frame/passes/foregroundLabelsLayer.test.ts` (extend); `src/services/gpu/shaders/starPoints/vertex.wesl` (modify — clamp `out.clip.z`; MANDATORY) plus, per decision (A), `src/services/gpu/shaders/labels/vertex.wesl` + `src/services/gpu/shaders/markerLines/vertex.wesl` (modify — clamp `clip.z` / `out.pos.z`); plus the star-label settings toggle (user-requested 2026-07-11) — a boolean settings field + SettingsPanel control + the layer reading it.

**Signatures (contract — match exactly):**
...
export function starCaptionFadeAlpha(input: {
  apparentSizePx: number;
  thresholdPx: number; // the shared Task 4 star-resolve threshold
}): number; // 0 well below the band, 1 well above, smoothstep across it
```

**Behaviour:** `declutterByScreenSeparation` sorts candidates by `priorityPx` DESC (stable input-order tiebreak — the director's convention, `labelDirectorSubsystem.ts:261-264`), greedily accepts a candidate when it is at least `minSeparationPx` from every already-accepted candidate, and returns the kept indices — pure, takes already-projected screen positions so it unit-tests without a device or a vp. `starCaptionFadeAlpha` is a `smoothstep` over an apparent-size band centred on the SAME `thresholdPx` the sphere partition uses (Task 4), returning `0` when the star is well below resolving, `1` when well above, and a fraction across the band — so a caption grows in continuously instead of popping. Pure; modelled on `milkyWayApproachFadeAlpha.ts`.

- [x] Add `declutterByScreenSeparation.ts` — single function, file named for it. Didactic docblock: WHY a separate pure cull and not the director's (the director's declutter is welded to `ctx.vp` + the merged producer set — the NEAR0 foreground captions can't join it, per Task 9's slab tension); WHY priority = apparent size (keep the near/bright star's caption, drop the distant one that would clutter — the director's own rationale, `labelDirectorSubsystem.ts:184-192`).
- [x] Add `starCaptionFadeAlpha.ts` — single function, file named for it. Didactic docblock: WHY a fade band and not a boolean gate (a boolean resolve gate pops the caption on/off as the camera crosses the threshold; a `smoothstep` band ramps the alpha so the caption grows in — the same continuous-envelope treatment the director gives famous labels, `labelDirectorSubsystem.ts:391,401`); WHY it shares the Task 4 star-resolve `thresholdPx` (one crossover point for the sphere AND its caption — do not fork the constant).
- [x] Test `starCaptionFadeAlpha ramps a mid-band caption to a fractional alpha` — an `apparentSizePx` inside the band returns a value strictly between 0 and 1 (NOT 0/1), and the below-band / above-band ends return 0 / 1 (pins the band moved off a boolean, per the Defect-1 requirement).
- [x] Test `declutterByScreenSeparation keeps the highest-priority of an overlapping cluster` — several candidates within `minSeparationPx`, differing priority → only the top survivor's index returned; a well-separated candidate is always kept.
- [x] In `foregroundLabelsLayer.draw`, compute per-star `apparentSizePx` from the star radius + the NEAR0 view (`view.camPos`, `ctx.fovYRad`, `view.viewportPx`), derive each caption's `fadeAlpha` via `starCaptionFadeAlpha` (Sun pinned to 1), DROP fully-faded captions (`fadeAlpha === 0`) from the declutter candidate set, project the survivors, `declutterByScreenSeparation` them, and set `fadeAlpha` on BOTH the surviving `Label` and its connector `MarkerLine` (both premultiply it in-shader). The per-frame set replaces the module-cached `BASE_LABELS` flush (`foregroundLabelsLayer.ts:111,147-214`); Earth + planets keep showing at full alpha (only the dense star map fades + declutters). The threshold is ONE named constant, shared with / tied to the Task 4 star-resolve threshold — do not duplicate.
- [x] Clamp `out.clip.z` in `starPoints/vertex.wesl` (MANDATORY — the always-on backdrop): `out.clip.z = min(out.clip.z, out.clip.w * (1.0 - EPS))` with `EPS` a named const, so the parsec-scale points survive any far plane on a deep descent. Didactic comment: WHY only the depthless NEAR0 overlay is clamped (the depth-tested bodies keep the tight `foregroundFrustum` far plane for precision — `foregroundFrustum.ts:26-37`), and WHY z-clamp is inert here (no depth test/write on the additive HDR point pass). Follow the `wesl-shaders` skill; no backticks in WESL comments.
- [x] Resolve the caption far-plane decision (A xor B above) and record which in the task/PR: EITHER clamp `clip.z` in `labels/vertex.wesl:72` + `out.pos.z` in `markerLines/vertex.wesl:69-72` the same way (note both passes are depthless OVER on every path, so the COSMO director labels are unaffected — verify visually), OR encode the coupling of the Defect-1 fade band's far edge to `foregroundFrustum`'s far plane (shared constant / cross-referencing comments) so an unresolved caption is `fadeAlpha ≈ 0` before its far plane can bite. Do NOT ship both.
- [x] Add a user-facing **star-labels toggle**: a boolean settings field defaulting ON (ground its home in the existing settings shape — find where comparable label/visibility toggles live in the settings slice and mirror that placement + naming; wire a SettingsPanel control the same way its siblings are wired), read by `foregroundLabelsLayer` so OFF suppresses the STAR captions only (Earth + planet captions unaffected). Test (extend `foregroundLabelsLayer.test.ts`): `foregroundLabelsLayer suppresses star captions when the toggle is off` — toggle off in the settings fixture → no star captions emitted, Earth/planet captions still present.
- [x] Test (extend `foregroundLabelsLayer.test.ts`): `foregroundLabelsLayer fades distant unresolved star captions` — a far camera where the star map is sub-resolve asserts the emitted star captions carry `fadeAlpha` near 0 (and fully-faded ones are dropped from the declutter set); a near camera emits more, at higher alpha. Mirror the layer test idiom.
- [x] `npm test -- declutterByScreenSeparation starCaptionFadeAlpha foregroundLabelsLayer` → green.
- [x] **VISUAL GATE (needs `?deepZoom`) — STOP and ask the user to confirm on the dev server:** the local-star captions no longer clobber into a pile from far out; each FADES in smoothly (no pop, no double-state) as its star nears/resolves, overlapping captions de-collide, and there is NO flicker at any distance. On a deep descent to ~1 AU scales the star-point field stays visible the whole way down (no far-plane clip), and each caption holds its minimum lift off the star (that lift ships in Task 9 — reference, not re-implemented here). Commit after the user confirms.
