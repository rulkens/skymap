...
- `SurfaceCutTile.edgeCoarser` carrying a 3-valued code per HEIGHT-level step, `2` exactly at band seams; `lattice.wesl`'s `latticeHeightM`/`latticeCellM` the one statement of post addressing for both stages.
