...
**Steps:**
- [x] **Frame profile (main thread runs this with the user).** Dev server + browser Performance panel: capture a fly-in over a dense region, compare against the ~4.2 ms two-planner baseline in `docs/backlog/2026-06-30-unify-disk-planner-walks.md`. Expect the merged walk's per-row geometry (`camDistSq`/`sqrt`/position read) to be computed once — roughly halving the shared geometry cost. Record before/after numbers. *(Outcome: measured headlessly with both shapes built from the shipped code — one shared walk driving both bodies vs two walks driving one body each, synthetic 2M-row catalog, M1 Max/Node: 1.03 ms vs 2.01 ms per frame, ratio 0.51 — the predicted halving.)*
- [x] **Visual parity (dev server).** Confirm procedural↔textured crossfades, the famous-WebP fade-out, and sticky behaviour look unchanged on close approach to a famous galaxy (the shared cursor changes WHICH frame a galaxy updates, not the steady-state image). Ask the user to look. *(Outcome: user confirmed 2026-07-10.)*
- [x] **entanglement-radar (main session runs the skill)** over the full diff — confirm the walk/bodies split is composed, not re-complected (no per-type branch re-grown, no mirror cursor, no megamorphic `onRow` site, `onRow` allocation-free). *(Outcome: clean — one low-severity note recorded: walk input and visitor `beginFrame` input must be the same object by call-site discipline; single call site + parity test make it moot.)*
- [x] All green, profile recorded, visuals confirmed → run `/feature-done`. *(Audit READY 2026-07-10.)*
