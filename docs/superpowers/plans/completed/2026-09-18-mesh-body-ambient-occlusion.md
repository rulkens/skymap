...
- [x] `BAKE_PASSES` rows (`meshPrebake.py:84-89`) gain a sample count; every existing
...
- [x] No test (no Python harness; behaviour-neutral). Commit
...
- [x] Test `the rovers bake against a ground whose up is source +Y`: `curiosity`,
      `perseverance`, `mer` → `[0, 1, 0]` (toBeCloseTo per component).
- [x] Test `a floating mesh bakes with no ground`: `voyager`, `hubble` → `undefined`.
- [x] No test for the mixed-key throw: no real data can reach it, and faking the tables
      would test the fake.
- [x] Commit `feat(meshes): derive which meshes sit on the ground from the scene data`.
...
- [x] `prebakeMesh.ts <key>`: `spawnSync` Blender (`process.env.BLENDER` ??
...
- [x] `package.json` `prebake-mesh` → `tsx tools/meshes/prebakeMesh.ts`.
- [x] `meshPrebake.py main()` (`:339`): parse the args after `--` as
...
- [x] No test (a process spawn; Task 7 exercises it). Commit
...
- [x] New last row in `BAKE_PASSES`: `("occlusion", dict(type="AO"), "Non-Color", None)`
      with `AO_SAMPLES = 128`.
- [x] `main()`: set the scene-level AO distance Cycles' AO bake reads
...
- [x] Seated only (`--ground-up` given): after `join_meshes` and before the bake loop,
      ...
      `flatten_materials`.
- [x] `flatten_materials` (`:272`): link the occlusion atlas into the Occlusion input of
...
- [x] `export` (`:306`): seated → set `obj["aoGroundUp"] = [x, y, z]` and turn
...
- [x] No test (Blender-only; Task 7 verifies). Commit
...
- [x] `GROUND_PLANE_SPAN` is no longer a free constant: side = `(1 + 2 ×
...
- [x] Seated only: after the `BAKE_PASSES` loop, bake the plane itself: AO, AO_SAMPLES,
...
- [x] Stamp `obj["contactDecal"] = {"centre": [..], "u": [..], "v": [..]}`: source
...
- [x] Verify: `npm run prebake-mesh -- mer`; the contact PNG is white at the edges and
...
- [x] `MeshBuildTarget` (`buildMeshes.ts:46`) gains `readonly groundUp?: Vec3`;
      `main()` (`:616`) fills it from `meshGroundUpSource(key)`.
- [x] `bake()` (`:440`): read `extras.aoGroundUp` from the GLB's nodes (at most one node
...
- [x] The `_mr` R rule: R survives only when `material.getOcclusionTexture()` is the
...
- [x] Test `refuses a GLB prebaked for a different ground than the scene seats it on`
      (stamped GLB, target without `groundUp`) and the reverse case in the same test.
- [x] Test `writes R = 255 when the mr texture carries no occlusion`: MR image with
      R = 0 and no occlusionTexture → every written `_mr` pixel has R 255, G/B kept.
- [x] Test `keeps R when occlusion is packed into the mr texture`: same image set as
      both → R preserved.
- [x] Update the existing fallback test (`buildMeshes.test.ts:417`) to expect R 255.
- [x] Commit `feat(meshes): carry baked occlusion in R of the mr map, 255 when absent`.
...
- [x] `envSplitSum` (`pbr.wesl:357`) gains a last parameter `ao: f32`. Diffuse × `ao`;
...
- [x] `fragment.wesl:48`: sample `.rgb`; R is `ao`, G roughness, B metallic; fix the
      comment above it. Pass `ao` at `:58`. `pbrDirectSphere` is untouched.
- [x] No new test; WESL link is covered by the existing shader compile tests. Commit
      `feat(meshBody): baked occlusion dims the environment light`.

## Task 7 (controller): re-bake, build, backlog

- [x] Sources: the worktree's `data/raw/meshes/<key>/` holds only README.md. For
      ...
      through into main).
- [x] `npm run prebake-mesh -- <key>` × 5; then `npm run build-meshes`. Note:
...
- [x] Inspect the `_mr` PNGs: rovers' undersides dark in R; floating meshes R < 255
      only in crevices.
- [x] Backlog: delete effort A from `docs/backlog/2026-09-18-mesh-body-ambient-occlusion.md`
