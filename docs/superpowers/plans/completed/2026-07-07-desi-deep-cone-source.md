...
**Goal:** A permanent new survey source — a 2.5° ultra-deep cone of DESI DR1 LSS spectroscopy through the Corona Borealis supercluster (~27k rows pre-dedup, z up to ≈3.5 / ~7,100 Mpc) — fetched, parsed, cone-filtered, cross-matched, encoded to a tier-agnostic `desi-deep.bin`, wired into the runtime behind a Settings toggle, and added to the R2 deploy set.
