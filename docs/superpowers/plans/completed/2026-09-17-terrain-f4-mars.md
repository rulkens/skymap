...
- **R5 Every rover sits at its current or final position**, so each HiRISE window shows the rover where it is or where it stopped. Opportunity per R1 and Perseverance per R4. **Curiosity: −4.82460, 137.38848 E**, sol 5016 (~2026-09-15), end-of-drive RMC 125_198, from `mars.nasa.gov/mmgis-maps/MSL/Layers/json/MSL_waypoints_current.json`. **Spirit: −14.60036, 175.52576 E**, at Troy on Home Plate's west edge (Arvidson et al. 2010, doi:10.1029/2010JE003633). This is a proxy, since no lat/lon fix is published: it is the rover's bright dot in JPL PIA12205 (HiRISE ESP_013499_1650), matched by eye onto the Gusev ortho, good to about 10 m in the ortho's own frame. The grey stretches are measured over these windows (0.1 / 99.9 percentiles, full resolution). Spirit's window fits the Gusev rectangles with 87 m to spare on the east, but both products are void in its easternmost ~300–400 m (the slanted strip edge), which falls back to Viking + MOLA.
