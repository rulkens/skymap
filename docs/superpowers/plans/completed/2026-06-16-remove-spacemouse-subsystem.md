...
- [x] `SettingsPanel.module.css`: drop "SpaceMouse connect" from the comment at `SettingsPanel.module.css:302`.
- [x] `useEngineSettings.ts`: remove the `DEFAULT_SPACE_MOUSE_SENSITIVITY` import (verify the import line), the `spaceMouseConnected`/`spaceMouseSensitivity` `useState` cells + their comment block (`useEngineSettings.ts:55-72`), the two `settings`-object fields (`useEngineSettings.ts:79-80`), the `input: { spaceMouse: { onConnectedChange: setSpaceMouseConnected } }` callback subsection + its comment (`useEngineSettings.ts:95-101`), the `setSpaceMouseSensitivity` return field (`useEngineSettings.ts:103`), and the two docstring bullets (`useEngineSettings.ts:23-26`). After removal, `settings` is `{ filamentCounts }` and `engineCallbacks` is `{ filaments: { onReady } }` — no `input` cluster.
- [x] `UseEngineSettingsReturn.d.ts`: remove `setSpaceMouseSensitivity` (`UseEngineSettingsReturn.d.ts:27`) and its docblock (`:22-26`) and the no-echo-setter sentence in the type intro (`:9`).
- [x] `UseEngineSettingsState.d.ts`: remove the `spaceMouseConnected` (`:49`) and `spaceMouseSensitivity` (`:56`) fields + their docblocks (`:44-56`) and the "two SpaceMouse fields" mention in the type intro comment (`:33`).
- [x] Delete `useSpaceMouseDevicePresence.ts` and `spaceMouseDevicePresence.test.ts`.
- [x] Run: `npm run typecheck` — Expected: clean. (main thread)
...
- [x] Run: `npm test` — Expected: green. (main thread)
- [x] Commit (the modified UI/hook/type files + two deletes). (main thread)
...
**Edits:**
- [x] `engine.ts`: remove the `createSpaceMouseSubsystem` import + its leading comment block (`engine.ts:107-113`); the `spaceMouse: createSpaceMouseSubsystem({ … })` construction block incl. its `cancelTween`/`onConnectionChange`/`onAxes` callbacks and comment (`engine.ts:332-349`); the four bespoke helpers `connectSpaceMouse`/`disconnectSpaceMouse`/`isSpaceMouseConnected`/`setSpaceMouseSensitivity` (`engine.ts:655-672`); the `state.subsystems.spaceMouse.destroy()` line (`engine.ts:710`); the `input: { spaceMouse: { … } }` handle literal (`engine.ts:855-862`); the `spaceMouseSubsystem.ts` bullet in the module-header subsystem list (`engine.ts:38`); the `SpaceMouse per-frame block` line in the tween-manager comment (`engine.ts:326`); and the `spaceMouse` mention in the `subsystems` construction comment (`engine.ts:180`).
- [x] `EngineSubsystemHandles.d.ts`: remove the `SpaceMouseSubsystem` import (`EngineSubsystemHandles.d.ts:26`), the `spaceMouse: SpaceMouseSubsystem` field (`:61`), and the `spaceMouse` mention in the "Eager (no GPU dep)" comment (`:10`). The trailing `_EnforceDestroyable` compile-time guard stays valid.
- [x] `EngineHandle.d.ts`: remove the `EngineInputHandle` import (`EngineHandle.d.ts:36`) and the `input: EngineInputHandle` field (`:60`). Per locked decision 1, do NOT leave an empty `input` stub.
- [x] `EngineState.d.ts`: drop "the SpaceMouse and" from the intro comment (`EngineState.d.ts:8`).
- [x] `EngineCallbacks.d.ts`: remove the `input` field (the `spaceMouse?: { onConnectedChange?: … }` subsection at `EngineCallbacks.d.ts:186-194`) — it only ever carried `spaceMouse` — and the SpaceMouse mention in the type's intro comment (`:27`).
- [x] `EngineSettingsCallbacks.d.ts`: drop `'input'` from the `Pick<EngineCallbacks, 'filaments' | 'input'>` union (`EngineSettingsCallbacks.d.ts:24`) → `Pick<EngineCallbacks, 'filaments'>`, and remove the `input.spaceMouse.onConnectedChange` clause from the comment (`:21`).
- [x] `defaults.ts`: remove `DEFAULT_SPACE_MOUSE_SENSITIVITY` + its `// ── SpaceMouse ──` section header + docblock (`defaults.ts:278-286`).
- [x] `settingsTable.ts`: remove the `setSpaceMouseSensitivity` bullet from the "bespoke setters" docstring (`settingsTable.ts:36`). Comment only — there is no table row.
- [x] `settingsTable.test.ts`: remove `setSpaceMouseSensitivity` from the bespoke-setters comment (`settingsTable.test.ts:36`). The asserted `names` list does NOT contain it, so no assertion changes.
- [x] `engineState.test.ts`: remove the `createSpaceMouseSubsystem` import (`engineState.test.ts:44`) and the `spaceMouse: createSpaceMouseSubsystem({ … })` entry from BOTH engineState fixtures (`engineState.test.ts:180`, `:379`).
- [x] Delete `EngineInputHandle.d.ts`.
- [x] Run: `npm run typecheck` — Expected: clean. (main thread)
- [x] Run: `npm test` — Expected: green. (main thread)
- [x] Commit (engine.ts + the modified types/defaults/tests + the one delete). (main thread)
...
- [x] `src/services/input/spaceMouse.ts`
- [x] `src/services/input/spaceMouseReport.ts`
- [x] `src/services/input/spaceMouseAxes.ts`
- [x] `src/services/input/spaceMouseSensitivity.ts`
- [x] `src/services/input/spaceMouseToCamera.ts`
- [x] `src/services/input/webhid.d.ts` (ambient WebHID shim — only used by `spaceMouse.ts`)
- [x] `src/services/engine/subsystems/spaceMouseSubsystem.ts`

**Delete — dedicated types (`src/@types/`):**
- [x] `input/SpaceMouseAxes.d.ts`
- [x] `input/SpaceMouseInputOptions.d.ts`
- [x] `input/SpaceMouseInputLike.d.ts`
- [x] `input/SpaceMouseInputCtorOptions.d.ts`
- [x] `input/SpaceMouseInputFactory.d.ts`
- [x] `engine/handles/EngineSpaceMouseHandle.d.ts`
- [x] `engine/subsystems/SpaceMouseSubsystem.d.ts`
- [x] `engine/subsystems/CreateSpaceMouseSubsystemInput.d.ts`

**Delete — dedicated tests:**
- [x] `tests/services/input/spaceMouseReport.test.ts`
- [x] `tests/services/input/spaceMouseSensitivity.test.ts`
- [x] `tests/services/input/spaceMouseToCamera.test.ts`
- [x] `tests/services/engine/subsystems/spaceMouseSubsystem.test.ts`
...
- [x] Run: `npm run typecheck` — Expected: clean. (main thread)
- [x] Run: `npm test` — Expected: green. (main thread)
- [x] Commit (all deletes, paths enumerated explicitly — never `git add -A`). (main thread)
...
- [x] Annotate/strike the "Remove the SpaceMouse … subsystem" surfaced-issue bullet (around `docs/BACKLOG.md:66`) as DONE (this PR). Match the file's existing convention for completed surfaced issues — check how prior done items are marked before editing.
- [x] **Do NOT touch** the "Home button in the top bar" bullet (unrelated), completed plans, or `CLAUDE.md`.
- [x] Run: `npm run typecheck` — Expected: clean. (main thread; trivial, confirms nothing slipped)
- [x] Commit (`docs/BACKLOG.md` only). (main thread)
...
- [x] `npm test` green (the suite count drops by the deleted SpaceMouse tests + the removed `input`-wrapper case; net is expected to fall — no new tests are added).
- [x] No `SpaceMouse`/`spaceMouse`/`webhid` symbol survives in `src/` or `tests/` except the optional incidental comment mentions listed in Task 4.
- [x] The `CameraDriver` registry is intact: `runCameraDrivers`, `buildCameraDrivers` (now `tween` + `autoRotate` only), `CameraDriver`, `RunFrameDeps.drivers`, the `runCameraDrivers(deps.drivers, …)` write site, and the `deps.drivers.some(…)` wake gate are all present and unchanged in logic. `cameraDrivers.test.ts` is untouched.
- [x] `EngineInputHandle` is gone (no empty stub); `EngineHandle` has no `input` field.
- [x] BACKLOG item marked done.
- [x] PR opened from `remove-spacemouse-subsystem` (base `main`).
