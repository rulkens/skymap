...
- [x] `npx tsx tools/volumes/buildRhizomeVolume.ts <cube.npy> --quick-look` writes `public/data/mcpm-large.scfd` and `public/data/mcpm-large.scfd.quicklook`, and prints both operational notes (tier must be **large**; `npm run build-mcpm` to restore). (verified live 2026-08-11 in an isolated worktree, pre-#545 layout: file + sentinel written — sentinel timestamped *before* the build — both notes printed, output byte-identical to the shipped reference)
- [ ] With the dev server running and the MCPM tier set to **large**, the reproduced cube renders in place of the reference — inferno palette, filament structure visible at the default contrast 1.7, positioned in the frame its sidecar declares. (cross-repo: needs the PolyPhy fork's real calibration cube, PR rulkens/PolyPhy#114 — and now also the v9 epoch-prefix alignment, see `docs/backlog/2026-08-11-rhizome-quicklook-v9-epoch-alignment.md`)
- [x] `npm run sync-r2` **refuses** while the sentinel is present, naming `npm run build-mcpm` in the refusal. (verified live 2026-08-11: exit 1, refusal names `npm run build-mcpm`, before any byte moves)
- [x] `npm run build-mcpm` restores the reference, removes the sentinel, and `npm run sync-r2` then proceeds normally. (verified live 2026-08-11: all three tiers rebuilt, restored large tier byte-identical to the shipped reference, sentinel cleared — which is the gate's only trigger; the sync run itself was not exercised from the worktree, deliberately)
