...
| `Source.Glade`, `Source.FamousGalaxy`                                | `magG` = B, `magR` = V | both finite: `log₁₀(M/L_B) = −0.942 + 1.737·(B−V) − 0.15`, luminosity in B | `M_B,☉ = 5.44` |
...
- [ ] Mass sanity, via a scratch `tsx` script that decodes `galaxy-catalog/v9/sdss-large.bin`, `2mrs.bin` and `glade-large.bin` — resolved with `resolveDataFile`, since the names on disk now carry hashes — and prints, per file: the finite-mass fraction and the 10th/50th/90th percentiles of `log10StellarMass`.
      Acceptance: SDSS median in **10.0–10.8** (the spec's ≈10^10.3 expectation, widened for the tier's bright M_abs cut); 2MRS median in **10.0–11.0**; GLADE median in **9.0–10.5**; every percentile finite; finite fraction > 0.9 for all three. `milliquas-large.bin` must be **0 %** finite.
