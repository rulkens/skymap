...
- [ ] Add the `fields` member to the `volumes` cluster; import `VolumeFieldId` (`@types/data/VolumeFieldId`) and `VolumeFieldSettings` (`@types/settings/VolumeFieldSettings`).
- [ ] Rewrite the `volumes` docblock (`:126-134`): drop "Per-field params … live on the volume store (`state.data.volumes`), not here" — state that per-field params now live here in `fields`, seeded from `SOURCE_REGISTRY` at construction.
- [ ] `npm run typecheck` → expected FAIL: the construction literal at `engine.ts:342` is missing `fields`, and `makeState()` in `tests/services/engine/wiring/settingsTable.test.ts:62` is missing `fields`.
- [ ] Commit: `feat(settings): add volumes.fields to EngineSettingsState`.
...
- [ ] In `tests/services/engine/wiring/settingsTable.test.ts`, extend `makeState().settings.volumes` to `{ masterEnabled: false, fields: {} }` so the fixture type-checks (the table itself doesn't touch `fields`).
- [ ] In `engine.ts:342`, seed `fields` inline from `seedVolumeFields()` — e.g. `volumes: { masterEnabled: DEFAULT_VOLUMES_ENABLED, fields: seedVolumeFields() }`. Import `seedVolumeFields` from `src/data/volumeFieldDefaults`. (`seedVolumeFields()` returns a `Partial<Record<…>>`; DEV-only debug fixtures are absent by design — matches today's store seed.)
- [ ] Delete the `for (… seedVolumeFields()) state.data.volumes.setParams(…)` loop at `:604-606` AND its `// ── Seed the volume store …` comment block (`:596-603`). The store seed is now redundant with the settings seed; the store itself is deleted in Step 3.
- [ ] `npm run typecheck` → expected PASS for the settings literal; the store still exists so its readers still compile.
- [ ] `npm test -- settingsTable` → expected PASS (frozen key test still 15; `makeState` now type-checks).
- [ ] Commit: `feat(engine): seed state.settings.volumes.fields at construction`.
...
- [ ] Add test `derives field values from state.settings.volumes.fields` — given a state whose `settings.volumes.fields['mcpm']` has `{ contrast: 3, intensity: 0.2, … }` and a renderer reporting handle `'mcpm'`, assert the row carries `contrast === 3` and `intensity === 0.2` (i.e. the value comes from settings, not the store).
- [ ] `npm test -- buildVolumeFieldsSnapshot` → expected FAIL (helper still reads the store).
- [ ] Repoint the `field` lookup to `state.settings.volumes.fields[id]`; keep the `?? defaults` fallbacks. Update the module docblock (`:13-16`): "merges the renderer's live handle list with the per-field bag from `state.settings.volumes.fields`".
- [ ] `npm test -- buildVolumeFieldsSnapshot` → expected PASS.
- [ ] Commit: `refactor(volumes): snapshot reads field values from settings`.
...
- [ ] Add `tests/utils/clampVolume.test.ts` with one test per helper asserting: in-range pass-through, above-cap clamp, below-floor clamp, and (where applicable) NaN → the documented fallback. Name them e.g. `clampVolumeContrast clamps to [0.05, 16] and passes mid-range`, `clampVolumeExposure maps NaN to 1.0`.
- [ ] `npm test -- clampVolume` → expected FAIL (helpers don't exist).
- [ ] Implement the five helpers, one per file. (`paletteId` and `enabled` need no clamp.)
- [ ] `npm test -- clampVolume` → expected PASS.
- [ ] Commit: `feat(volumes): extract per-knob clamp helpers`.
...
- [ ] Add `tests/services/engine/volumeFieldSetters.test.ts`. Build the engine (or the minimal state + setters) and for each setter assert, with names like `setVolumeFieldContrast writes settings copy-on-write, echoes snapshot, requests render, calls no renderer setter`:
  - the new value lands in `state.settings.volumes.fields[id]` (clamped where applicable — assert `setVolumeFieldContrast(99)` lands `16`);
  - `state.settings.volumes.fields` is a **new object reference** (copy-on-write, not in-place mutation);
  - `onFieldsChanged` fired once and `requestRender` fired once;
  - the renderer's value setters were **not** called (assert via a spy/fake renderer — `setEnabled`/`setContrast`/… not invoked);
  - an unknown id is a no-op (no throw, no callback, no render).
  - `setVolumeFieldEnabled(id, true)` also kicks `fades.fadeTo` and `maybeLazyLoadDebugVolume`.
- [ ] `npm test -- volumeFieldSetters` → expected FAIL (setters still write the store + call the renderer).
- [ ] Rewrite all 7 setters to the sketch shape, using the Task 2.1 clamp helpers. Drop every `state.data.volumes.params/setParams` line and every `state.gpu.scalarVolumeRenderer?.set*` value-call from these setters. Keep the `setVolumeFieldEnabled` fade + lazy-load lines.
- [ ] `npm test -- volumeFieldSetters` → expected PASS.
- [ ] Commit: `refactor(engine): volume setters write settings, not the store + renderer`.
...
- [ ] Add tests: `addVolumeField seeds a missing settings row and adds the GPU field without replaying renderer setters`; `addVolumeField preserves an existing settings row`; `removeVolumeField drops the settings row and removes the GPU field`.
- [ ] `npm test -- volumeFieldSetters` → expected FAIL.
- [ ] Rewrite both functions; the settings-row delete is copy-on-write (no `delete` on the live object). Update the inline comments to current behaviour.
- [ ] `npm test -- volumeFieldSetters` → expected PASS.
- [ ] Commit: `refactor(engine): add/removeVolumeField own settings rows, not the store`.
...
- [ ] Update / add a test `volumeField reads from state.settings.volumes.fields` — assert flipping `settings.volumes.fields['mcpm'].enabled` flips the predicate's read; an unregistered id returns `undefined`. Adjust the existing `demandCtx.test.ts` fixture from a `data.volumes` stub to a `settings.volumes.fields` record.
- [ ] `npm test -- demandCtx` → expected FAIL (still reads the store).
- [ ] Repoint the closure + comment.
- [ ] `npm test -- demandCtx` → expected PASS. Also run `npm test -- demandTable` (its fixture may seed `data.volumes`; repoint it to `settings.volumes.fields`).
- [ ] Commit: `refactor(volumes): demand predicate reads field settings`.
