...
- [ ] **TourActions adapter** (engine.ts, near the other internal helpers). `focus` dispatches on `TourFocus.kind`:
  ...
  - `'structure'` → resolve the id from the structure store (the data-layer `state.data.structures`; if it exposes no by-id getter, fall back to a `buildStaticAnchorPois()` id→record map built once) then `commitFocus`/`commitPoiFocus` (`helpers/commitPoiFocus.ts:39`). Missing id → skip silently (a renamed slug is a catalog bug, not a tour crash).
