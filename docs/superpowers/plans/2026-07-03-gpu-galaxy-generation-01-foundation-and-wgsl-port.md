...
- **Thread → output slot.** Each population owns a contiguous slot range carved CPU-side (`carveStarLayout` / `carveDustLayout`). A slot maps to `(iteration, sub)` via a per-population `stride`: spiral arms stride **5** (regular star or HII halo / HII core / up to 3 newborns), irregular clumps stride **2** (star-or-halo / core), everything else stride 1. Sibling sub-threads recompute the iteration's shared hash draws identically and each writes only its own slot — that recompute trick is also how the dust passes reconstruct the arm/clump dust _seeds_ without any cross-pass buffer.
  ...
  - DEVIATION: `armDust`/`irrDust`'s CPU hard cap (`dust.count() < budget`) is inherently serial — a shared counter thousands of GPU threads can't advance. It becomes **resample-to-budget**: one thread per OUTPUT particle (`iterations = budget`, no candidate cap), each thread hash-picking an arm/clump candidate over the `spiralArms`/`irregularClumps` iteration space (read from the shared `starRanges` table) and resampling up to **32 attempts** until it lands a candidate that clears the seed + dense/keep gates. Hash-picking uniformly then gating on `seedAccepted` reproduces the CPU's compacted-`dustSeeds` distribution, and dispatching exactly `budget` threads pins the live total at ~budget — matching the CPU in the **budget-limited** regime (the old `min(armStarCount, budget)` cap under-emitted there: default Sc wrote ~1,110 vs CPU ~6,614, acceptance ~17%; resample now gives 6,597 vs 6,614). Two small artifacts: **candidate duplication** (two output threads may pick the same seed — identical sample, distinct jitter keyed on the output index; ~6% of particles at default params) and a **<0.3% shortfall** (32 attempts all miss, `(1−0.17)^32`). **KNOWN larger deviation — seed-limited over-emission:** the CPU actually emits `min(budget, seedsPassingKeep)`; resample matches only when `seedsPassingKeep ≥ budget`. Barred spirals starve the arms of dust seeds (measured SBb: ~7,200 seeds, ~4,440 pass keep, below budget 6,614), so the CPU is seed-limited while resample fills the budget — GPU over-emits arm dust ~+30% at default scale (up to +116% at dust 1.0 / 400k stars). The crossover for barred spirals sits above ~650k stars, so barred galaxies over-emit at every realistic star count. A stateless pass with no atomics/compaction (spec-rejected) cannot compute `min(budget, seedsPassingKeep)` exactly; resample-to-budget is exact where the CPU is budget-limited (common spiral case, Sc/Irr) and over-emits where it is seed-limited (barred). **RESOLVED (controller, 2026-07-06): ACCEPTED as the documented deviation.** The measured alternatives are strictly worse where it matters most: a strided no-replacement partition (Task 8c, reverted) fixes SBb but under-emits every budget-limited preset by the occupancy factor B·(1−e^(−A/B)) — measured −25% on the DEFAULT Sc — and the exact fix (a deterministic prefix-scan pass reproducing min(budget, seedsPassingKeep)) is the compaction machinery the spec rejected. Escalation path: if the plan-02 visual gate finds barred-preset dust visibly too dense vs the reference photos, build the prefix-scan pass then; the gate is the arbiter. Full analysis in .superpowers/sdd/task-8b-report.md + task-8c-report.md (gitignored scratch — the numbers that matter are restated here).
...
| 8   | armDust            | dust  | 1      | output particle (resamples arm-loop candidates)          |
| 9   | barDust            | dust  | 1      | particle                                                 |
| 10  | lenticularNucDust  | dust  | 1      | particle                                                 |
| 11  | lenticularRingDust | dust  | 1      | particle                                                 |
| 12  | irregularDust      | dust  | 1      | output particle (resamples clump-loop candidates)        |
...
- armDust (spiral/barred with `armStarCount > 0`): `floor(30000·dust / g²)` (`armDust.ts:26` — the CPU push-budget, one slot per particle; the GPU resamples per slot, so no candidate cap. Gated on `armStarCount > 0` to mirror the shader, which reads its candidate space from the `spiralArms` star range); barDust (barred only, i.e. barLength > 0): `floor(9000·dust / g²)` (`barDust.ts:20`); lenticularNucDust (lenticular): `floor(12000·dust / g²)` (`lenticularDust.ts:32`); lenticularRingDust (lenticular, `dustRingStrength > 0`): `floor(34000·ringAmt / g²)` (`lenticularDust.ts:55`); irregularDust (irregular with `armStarCount > 0`): `floor(16000·dust / g²)` (`irregularDust.ts:21`).

**Steps**

- [x] Failing tests — carveStarLayout: `Sc: bulge, disk, arms ranges are contiguous with arms stride 5` (assert each start = previous start + iterations·stride); `SBb: bar takes floor(diskCount*0.35) iterations and disk shrinks by the same amount`; `Irr: clumps range has stride 2 and there is no disk or arms range`; `E3: bulge and halo only`; `globularCount 12 appends a 1080-slot globularStar range`; `capacity equals the sum of iterations*stride`. carveDustLayout: `Sc default params: armDust iterations = floor(30000*dust/grainScale^2)` (the budget, no candidate cap); `armDust is gated on armStarCount > 0` (armStrength 0 → no dust ranges); `elliptical or dust 0 gives an empty layout with capacity 0`; `S0 with dustRingStrength 0 has only the nuclear range`; `S0 with dustRingStrength 0.5 adds the ring range with floor(34000*0.5/g^2) iterations`; `SBb has armDust then barDust`; `Irr has only irregularDust sized to floor(16000*dust/g^2)`.
...
armDust (8) — `armDust.ts:28-51`; **resample-to-budget**: thread `j` is one output particle. It loops up to 32 attempts, each attempt owning a disjoint 64-slot hash block (`base = 64·k`); the block draws a candidate pick at `base + 10`, hash-picks `c` over the `spiralArms` iteration space, recomputes `armStarSample(c)` (dead-`continue` if `!seedAccepted`), and runs the pipeline below at block-relative slots (`base + slot`, keyed on the output index `j`); the first candidate that also clears the keep gate emits. Inner-edge nudge inX/inZ are draw-free. Slots are block-relative; reddening stays at the fixed 56-59 window (above every block's pipeline slots, so no alias):
...
irregularDust (12) — `irregularDust.ts:23-38`; same **resample-to-budget** shape as armDust: thread `j` loops up to 32 attempts, each a 64-slot block (`base = 64·k`) with the candidate pick at `base + 10` over the `irregularClumps` iteration space, recomputes `irregularSample(c)` (dead-`continue` if the seed gate failed), emits on the first that also clears keep. Block-relative slots: 0 dense, 1-2 x genNormal, 3-4 y genNormal, 5-6 z genNormal, 7 keep, 8 size, 9 opacity; reddening at the fixed 56-59.
