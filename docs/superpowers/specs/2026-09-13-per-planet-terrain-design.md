...
//   (amended, F2-R1: a 3-valued code per edge — 2 = a band seam, skirted never collapsed)
...
2. **A 2:1 balance constraint**, so a leaf never neighbours a leaf more than one
   level away. This is what bounds `edgeCoarser` to a 3-valued code per edge
   (amended, F2-R1: `0|1|2`, packed 2 bits — not the one-bit-per-edge shape this
   section originally specified) and lets §7's stitching be a vertex-shader
   decision needing no neighbour data beyond one `u32`.
