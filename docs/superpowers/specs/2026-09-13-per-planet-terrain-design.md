...
//                                  + heightBoundsM; + edgeCoarser: [0|1|2, 0|1|2, 0|1|2, 0|1|2]
//   (amended, F2-R1: a 3-valued code per edge — 2 = a post-balance step of two or more, a band seam, skirted never collapsed)
...
| 7   | 2:1-balanced refine, so `edgeCoarser` is a 3-valued code per edge (amended, F2-R1) | growth          | `cutSurfaceTiles.ts:212-231` has no balance constraint                                                                                                                                                                                                                                                                                                                                                                                                     |
...
2. **A balance constraint on the HEIGHT level** (amended, R14, R12 kept), so no two
   ...
   bounds `edgeCoarser` to a 3-valued code per edge (F2-R1: `0|1|2`, packed 2 bits;
   2 = a step of two or more left by R12's band-ceiling exemption, a seam F2 skirts)
   and lets §7's stitching be a vertex-shader decision needing no neighbour data
   beyond one `u32`.
   ...
   with that edge's code at `2` (amended, F2-R1: skirted, never collapsed); §7.3
   says how F2 hides it.
