...
because the one demand it carried (`pgcAlias` on palette open) becomes a settings read (D6);
the `famousGalaxiesMeta` getter,
...
**This section now describes (e), not (d)** — plan 04e deferred it from `galaxyCatalog`'s own PR
to the first sibling Layer's, where dropping a Layer actually drops passes and targets; see §14's
row on the deferral.
...
src/layers/galaxyCatalog/
  ...
  load/         slot wiring, dissolveCatalogBuffer, pgcAlias + famousMeta slots
...
6. **D6 — no Layer handles; one facts mechanism.** `EngineHandle` is `{ destroy, debug }` and the
   ...
   instead. `EngineData` dissolves with them:
...
| D5: the ratchet's one exemption "becomes the composed-tuple import once D1 lands"               | wrong. `settingsSlice.ts` keeps its thirteen direct fragment imports: the `liftClusterReducers` spreads need each fragment's literal type for the typed `settingsSlice.actions` exports, so the inbound ratchet row stays `state/settings/settingsSlice: 13`, not 1, until reducers compose at the type level (PR-B, plan 04b Findings row 5).                                                                                                                                                                                                                                                                                                                      |
| P7 deletes `engineFamousGalaxiesMetaReported` and `engine.meta.famousGalaxies` in PR-C          | they ride PR-D. `CommandPaletteContainer` is the shell's only reader of the famous meta (`selectFamousGalaxiesMeta`) and has no other channel until PR-D publishes the `famousMeta` fact, so §9(d)'s own rule — a deletion rides the PR that lands its replacement — keeps all three alive for one PR, during which the sidecar slot writes both homes (plan 04c Ruling 4).                                                                                                                                                                                                                                                                                         |
| P7 adds `composeSources` in PR-C                                                                | PR-D adds it, with its first consumer. Over `layers: []` the composed record is `{}`, so a PR-C `composeSources` would be an unconsumed function. What PR-C needs is the fold from a rows tuple to a code-keyed record, and it ships as `sourceRecordOf(rows)`, consumed by `SOURCE_REGISTRY`; PR-D's `composeSources` is `sourceRecordOf(layers.flatMap((l) => l.sources ?? []))` (plan 04c Ruling 5).                                                                                                                                                                                                                                                             |
| §9(d) re-homes `SOURCE_REGISTRY` into the Layer and adds `composeSources` in PR-D               | neither happens in (d). Ten `src/data/` modules read galaxy entries at module init, so a registry composed from `APP_COMPOSITION` would sit on the cycle `data -> compositions/app -> layer.ts -> render/* -> data/sources`, whose first `tsc`-invisible symptom is a TDZ throw at boot. `layer.ts` declares `sources` from 04c's leaf rows file instead — one authority, two readers — and the move waits on the ten readers (plan 04d Ruling 1).                                                                                                                                                                                                                  |
| D13 gives the Layer a `ui` contribution in PR-D                                                 | it waits for (e). `layer.ts` importing `GalaxiesSectionContainer` closes `settingsSlice -> appSettingsFragments -> APP_COMPOSITION -> layer.ts -> container -> store hooks -> settingsSlice`; the mechanism 04b shipped is fine, the cycle is in what the container imports. `SettingsPanel` keeps rendering the section by hand and (e) lists the sections without `layer.ts` importing components (plan 04d Ruling 8).                                                                                                                                                                                                                                            |
| §9(d) publishes only `famousMeta` in PR-D, with the other facts later                           | `provenanceCounts` lands with it. What forces publication is where the PUBLISHER lives: the galaxy slot's `subscribe` is the sole dispatcher of both `engineFamousGalaxiesMetaReported` and `engineProvenanceCountsReported`, and it moves into the Layer in (d), so both redux copies die there. `aliasIndex` and `structureMemberCount` stay (e)'s — their builder and their reconcile are still shell-side (plan 04d Ruling 12).                                                                                                                                                                                                                                 |
| D5: a formed Layer's settings reach `settingsSlice` through the composed tuple alone            | the Layer's settings root is imported twice on purpose — by `layer.ts` and by `appSettingsFragments.ts`. Reaching the fragments through `layer.ts` closes a `tsc`-visible circular type alias (`settingsReducer` -> `EngineSettingsState` -> `APP_SETTINGS_FRAGMENTS` -> the composition -> `layer` -> `ContentPass` -> `PassState` -> `EngineState`). The ratchet row stays at 13 either way, and `SettingsOf` is deleted with the cycle (plan 04d, D2).                                                                                                                                                                                                           |
| the Layer keeps a synthetic backstop, armed by a predicate over the point slots (D6, Ruling 13) | superseded. The synthetic galaxy catalog is deleted wholesale (user ruling 2026-09-16) — `Source.Synthetic`, its entry, its fetcher, its generator and every special case. A catalog that fails to load sits in its slot's `error` state and `installFormatVersionAlert` still raises the stale-`.bin` splash; there is no replacement and no arming predicate.                                                                                                                                                                                                                                                                                                     |
| §7/§10(d): `galaxiesOnly` (the reference engine) ships in PR-D, with `galaxyCatalog`            | it moves to §10(e), with the first sibling Layer (plan 04e scope ruling, user ruling 2026-09-16). `CONTENT_PASSES` (`frame/passes/index.ts:50-87`) still holds 36 non-galaxy passes — Milky Way, stars, structures, bodies, volumes, constellations, atmosphere — so `layers: [galaxyCatalogLayer]` is behaviourally identical to `APP_COMPOSITION`: no render target drops, no `FRAME_ORDER` name falls away, until a second Layer forms. §7's bullet 3 (a root settings type holding only `galaxyCatalogs`) also needs a composition-parameterized `createAppStore`/`settingsSlice`, which 04b did not build. §7 itself is otherwise accurate for (e) as written. |
| §9(d)/D9's `diskRadiusRing` is the one Layer renderer left owning its own swap-format tracking  | the overlay it was written for no longer exists: `diskRadiusRing`, `diskRadiusRingPass` and the `disk-radius-ring` debug-overlay row were deleted wholesale in 04e (deletion-audit finding, user ruling 2026-09-16) — ~575 LOC for one DebugPanel checkbox. D9's rule (a Layer renderer that draws to the canvas tracks its own format at draw, no `swapFormat` hook) stands for the next renderer that needs it; its worked example is now history, not a live contract.                                                                                                                                                                                           |
