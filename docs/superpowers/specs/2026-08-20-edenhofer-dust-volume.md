...
Ideal shape: one source row (`edenhofer-dust`, NEAR0-rendered), with `VolumeFieldDefaults` growing an optional `fadeBands?: readonly FadeBand[]` field (default `[surveyDeepZoom]`) — and `VolumeFieldSettings` growing `bands` seeded from it, so band edges are generically tunable via `writeVolumeField` (discharges grill Q8's "tunable" with zero dust-special code); the dust Layer's `DensityFieldTuningRow`-shaped row (`src/layers/cosmicWebDensity/ui/DensityFieldTuningRow.tsx`) simply omits the palette slider rather than conditionally suppressing it. `settings.dust = { rv, targetScale }` mirrors the `MilkyWaySettings`/`MILKY_WAY_SLIDER_FIELDS` debug-section pattern. The target row uses the live-scale precedent (`scale: (s) => s.settings.dust.targetScale`, mw-aggregate style).
...
**Two Layers, two instances (updated 2026-09-23 for the `cosmicWebDensity` Layer refactor):** `cosmicWebDensity` now owns its own `createVolumeFieldRenderer` instance, `cosmic-web-density` target, upsample and `createScalarVolumePass` row, wired entirely inside its own `create.ts`/`passes/`/`load/` — no shared `state.gpu.volumeFieldRenderer` or cross-Layer upload path survives for a second Layer to hook into. The dust Layer is therefore a **sibling instantiation**, not a routing problem: it mints its own `createVolumeFieldRenderer` (passed a `blend: 'multiply'` parameter the additive pair doesn't use), its own target, upsample and `createScalarVolumePass` row, and its own ingest in its `load/` — mirroring `cosmicWebDensity`'s shape end to end. No `absorptive` routing flag, no shared renderer to bypass: each Layer's `CosmicWebDensityFieldId`-style id union (Q9, `docs/grill-sessions/cosmic-web-density-layer-2026-09-22.md`) is already closed over its own registry rows, so a dust source row is invisible to `cosmicWebDensity`'s ingest and renderer from the moment it's registered — the former ordering hazard (a live slot before routing exists uploading into the wrong renderer) no longer has a mechanism to occur through.
