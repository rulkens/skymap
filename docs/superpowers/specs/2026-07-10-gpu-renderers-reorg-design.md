...
`src/services/gpu/renderers/` is **17 flat files** ranging 203–834 lines. Flat
is fine at five files; at seventeen it stops telling you which renderers are
_one conceptual thing split across files_ (the galaxy LOD chain) versus
_genuine singletons_ (the horizon shell). This reorganization does four things:
...
3. **Split `pointRenderer.ts`** (834 lines, 13 methods, the system's first
...
- **`debugSphere/`** — its eventual family (the zoom-to-earth plan-02 solar-
  system _bodies_) doesn't exist yet. It gets a one-file folder now and becomes
  `bodies/` when plan 02 lands (see §10).
...
src/services/gpu/
  ...
  renderers/
    ...
    debugSphere/          debugSphereRenderer.ts  → renamed bodies/ when zoom-to-earth plan 02 ships
```
...
packed with a byte-identical 5-line write in **five** files:
...
Every site is the same three-op sequence: `uni.set(viewProj, 0);
...
`ADDITIVE_BLEND` (8 byte-identical sites — the `one`/`one`/`add` descriptor
...
pipeline, `milkyWayCloudRenderer`'s `STAR_BLEND`, and `instancedQuadRenderer`'s
additive branch) + `PREMULTIPLIED_OVER_BLEND` (5 byte-identical sites —
...
`pointRenderer.ts` is 834 lines and its return object exposes **13 methods**
...
- **Kill the `setBuildBufferRunner` module-global.** The worker runner is
  ...
  (`pointRenderer.ts:284`) — a test-injection hatch. Make it a **`catalogStore`
...
`package::points::io` becomes `package::galaxyCatalog::points::io`, etc. —
roughly a dozen `import` lines across the `.wesl` files (5 files reference
`package::points` today) plus ~9 `?static` TS import lines.
...
| Surface                                              | Count                           | Where                                                                                            |
| ---------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------ |
| `?static` shader imports gaining one `../`           | **39** across 16 renderer files | every `renderers/*.ts` that imports a `.wesl`                                                    |
| `vi.mock('.../renderers/<name>')` literals           | **16** (15 + 1)                 | `tests/services/engine/phases/initGpu.destroyReachability.test.ts` (15), `wireInput.test.ts` (1) |
| Renderer test files moving to mirror `src`           | **19**                          | `tests/services/gpu/renderers/*`                                                                 |
| `initGpu.ts` renderer-factory imports                | **15**                          | `phases/initGpu.ts:46–66` (pick lives in `wireInput`, not here)                                  |
...
`debugSphereRenderer` is the **only renderer with zero tests** (violates the
`renderers.md` checklist's "at least one test that exercises construction + a
representative call" — confirmed: no `debugSphereRenderer.test.ts` exists). Add a
construction + `draw` test **during its folder move**.
...
3. **Folderize**: `git mv` into family folders + mechanical import / `vi.mock`
   rewrites + shader nesting under `shaders/galaxyCatalog/` + `gpu/labels/` →
...
- **Step 3 additionally** gets a dev-server **visual smoke check**: galaxies,
  ...
  cloud + pick, volume field, flow field, and pick/hover — because ~40 import
  lines of shader plumbing move and the iOS-shader landmine means a broken
  pipeline can present a black canvas with no thrown error.
- The reorg is **behaviour-neutral**: the only runtime changes are the
  ...
  characterize beyond the `debugSphereRenderer` construction/draw test added in
  §10.
...
- Zoom-to-earth plan 02 (PR #423) — the predecessor this reorg sequences after;
  its `bodies` renderers become `debugSphere/`'s eventual family.
