...
correctly the runtime needs per-galaxy *placement calibration* — the
...
- **It is famous-only.** SDSS / 2MRS / GLADE rows never carry it. Of the
  ...
  a calibration, and most of *those* don't (the field is optional — an
...
- `famous.bin` — the fixed-stride `GalaxyCatalog` binary (position,
  ...
  point/billboard renderer. Same v6 64-byte stride as every other tier,
  with [only a few spare bytes per row](../../src/data/galaxyCatalogFormat.ts).
...
- **(a) A new `famous_calibration.json` sidecar.** Rejected: it
  ...
  *already* holding. A second famous-only JSON keyed by the same id is
...
- **(b) A `famous.bin` v7 format bump.** Rejected on three counts:
  ...
  - A stride change is a **regenerate-all**: the on-disk format header is
    ...
    R2 re-sync.
  - It is a semantic mismatch: `famous.bin` is a galaxy *vertex-buffer*
    consumed by the GPU point renderer; calibration is a curator render
    *hint* consumed CPU-side by the thumbnail planner. Optional,
...
- `src/data/galaxyCatalogFormat.ts` — the v6 fixed-stride format a bump
