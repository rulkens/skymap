...
Each `build-*` script above already tails with `npm run build-data-manifest`, which writes `public/data/manifest.json` **last**, after every file it names, so a run always leaves one coherent manifest. Baked mesh-body assets (`npm run build-meshes` — per-tier `<key>-<tier>.mesh` (e.g. `curiosity-small.mesh`) plus its three PBR WebPs, a contact-shadow WebP for seated keys, and an untiered `<key>_hole.webp` terrain-hole mask for a `georeferenced` source, whose surveyed anchor places the scan on its site, under `public/data/meshes/`) follow the same rule: `allowDataFile` tracks them by name, so they ride the ordinary `public/data` group in `sync-r2-secure` alongside the catalog tiers, with no group of their own to configure. A hand-run `tsx tools/…` invocation skips that tail and must be followed by the pass manually — `sync-r2-secure`'s drift guard refuses to sync when the tracked set doesn't match the manifest.
