...
- **Audit first, then policy** — instrument/inventory: startup loads vs on-demand, bytes per asset (glade-large ≈ 130 MB), what a tier switch leaves behind (does the old tier's bin/GPU buffers linger?), volumes after layer-off.
