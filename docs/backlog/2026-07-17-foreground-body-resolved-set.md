...
once in `drawPick` — because `ContentPass` has no shared per-frame scratch. The cull
...
(`starSpheresPass.{draw,drawPick}` + `starPointsPass.{draw,drawPick}`). A per-layer
...
Design-bearing: touches the `ContentPass` shape / frame-context wiring. Guarded for now
by `passes.test.ts` (nine-pickable pin) + the `starSpheresPass.drawPick` seed-index
regression.
