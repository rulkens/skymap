...
The known-gap note in `starSpheresPass.ts:21-30` describes the intra-layer case —
two simultaneously-resolved scene stars in that layer's loop
(`starSpheresPass.ts:119-129`). There is a second, structurally independent site:
`fieldStarSpherePass.draw` calls the same renderer for the nearest resolvable Gaia
field star (`fieldStarSpherePass.ts:240-258`).
...
- `starSpheresPass` draws only the `spheres` branch of
...
- `fieldStarSpherePass` needs the camera within solar-radius resolving range of a
  catalogued Gaia star (`fieldStarSpherePass.ts:217-238`), which is nowhere near
...
first:** instancing requires one batch, and two independent `ContentPass` rows
cannot pack into a shared instance buffer without one of them learning about the
other — which is exactly the braid `fieldStarSpherePass.ts:51-66` argues against
...
- `src/services/engine/frame/passes/starSpheresPass.ts:21-30,119-129` — call site
  one and the known-gap note.
- `src/services/engine/frame/passes/fieldStarSpherePass.ts:240-258` — call site
