...
`computeStarCut` (`src/services/gpu/renderers/starCatalog/cut/computeStarCut.ts`)
bakes every octree node's position relative to a single camera position taken
once, at prepare time:

- It reads `ctx.drawCamPos` as the walk's camera position.
- Its `emitNode` closure bakes each node's `originRelCamMpc` as
  `nodePc·pcToMpc − camPos`, i.e. relative to that same `ctx.drawCamPos`.
- The result is memoised per-frame on `ctx` (`preparedByCtx` in
  `prepareStarCut.ts`), so the whole walk + bake runs once regardless of how
  many layers call it.

Draw time re-derives its own view-projection independently, per view:

- `drawStarStream.ts` and `drawStarPick.ts` both compute
...
2. Change `drawStarStream` and `drawStarPick` to rebase about `prep.originMpc`
...
doing only if a second load-bearing call site of this pattern shows up; the
star catalog's own `drawStarStream`/`drawStarPick` pair is the only one today.
