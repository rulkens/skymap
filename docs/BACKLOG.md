...
- [ ] **Synthetic fallback cloud never evicts** `needs-verification` — the synthetic row has a `demand` predicate and no `release`, so the backstop stays resident once a real catalog lands; surfaced by the Quest VR spike. → [details](backlog/2026-08-23-synthetic-cloud-eviction.md)
