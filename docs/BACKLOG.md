...
- [ ] **Synthetic fallback cloud never evicts** `needs-verification` — the `Synthetic` row has a `demand` predicate and no `release`, and the request flag that arms it is only ever set, never cleared; surfaced by the Quest VR spike. → [details](backlog/2026-08-23-synthetic-cloud-eviction.md)
...
- [ ] **`famous_stars_meta.json` demands unconditionally at boot** `awaiting-decision` — 118 KB on every visit for a sidecar only the InfoCard reads; parked 2026-09-01: the `pgcAlias` one-shot precedent is engine-internal `RequestKey` (ADR 0005 §3, deliberately outside Redux), user wants an RTK-shaped path — needs an architectural ruling before pickup.
...
- [ ] **Layer blend is declared twice** `needs-design` — `ContentPass.blend` and the pipeline's `GPUBlendState` restate each other with nothing tying them; parity wants `blendStateOf` threaded through every renderer. → [details](backlog/2026-07-31-layer-blend-declared-twice.md)
- [ ] **Frame-assembly walker needs blend-legality + target-format-parity validation** `needs-design` — `ContentPass.blend` is advisory and render-target formats are hand-matched at construction; neither is checked against the baked pipeline. → [details](backlog/2026-08-20-frame-assembly-blend-format-validation.md)
...
- [ ] **Rotation table has one shape; a second orientation kind needs a tagged union** `needs-design` — `RotationElements` assumes every body spins about a fixed IAU pole; a look-at or surface-locked orientation can't be expressed. → [details](backlog/2026-09-10-rotation-table-tagged-union.md)
- [ ] **Kepler solver has no hyperbolic branch** `needs-design` — `eccentricAnomalyFromMean` is bound-orbit-only (`e ∈ [0,1)`); blocks Voyager or any escape-trajectory body. → [details](backlog/2026-09-10-hyperbolic-kepler-branch.md)
- [ ] **No position driver for a body fixed to a host's rotating surface** `needs-design` — `deriveBodyStates` only knows orbits and fixed heliocentric anchors, not a lat/lon locked to a spinning host. → [details](backlog/2026-09-10-surface-fixed-position-driver.md)
