...
- [ ] **Bloom fill-cost trim** `needs-design` — bloom is ~5 ms / 23% on solar-system; the `BLOOM_LEVELS` 5→3 lever is measured-dead (spike 2026-07-22, zero change), cost is fill on the finest mips — shrink `bloom0`/`bloom1` scale or gate instead. → [details](backlog/2026-07-21-bloom-mip-count-perf.md)
