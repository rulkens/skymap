...
- [ ] **Decomplect the galaxy tool's probe API** `ready` — 14 `request*` debug methods outnumber the production surface on `GalaxyEngineHandle`, each declared thrice; fold into one `probe` sub-object + one generic slot-range peek. → [details](backlog/2026-08-12-probe-api-decomplect.md)
