...
- **`cluster*` → `structure*` naming migration** — surfaced 2026-06-04 while adding `group`. The featured-structure seam is named after one of the four categories it now holds (cluster / supercluster / void / group): the seed file `data/cluster_anchors.seed.json`, the `rawDataRegistry` key `clusters.seed`, `parseClusterSeed` / `ClusterSeedEntry` / `validateClusterSeedEntry`, plus `clusterFocusSubsystem`, `clusterCatalogSlot`, `clusters.ccat` / `clusters_meta.json`. The rest of the codebase moved to `Structure*` vocabulary in #253/#254 (`StructureCategory`, `StructureRecord`, `structures` store, `structurePoiStyles`, `produceStructure*`), so these are the holdouts. **Done so far:** the marker renderer (`clusterMarkerRenderer` → `structureMarkerRenderer`, its descriptor/handle/pass + the `clusterMarker/` shader module). **Remaining:** rename to `structure_anchors.seed.json` / `parseStructureSeed` / `StructureSeedEntry` / `structureFocusSubsystem` / `structureCatalogSlot` etc. Pure clarity (no behaviour change), but touches a tracked data file (`git mv` + `.gitignore` exception update), the registry key, the parser, and consumers (`buildClusters.ts`, audit scripts) — own focused diff, pairs with the identity consolidation above. Mechanical; typecheck + tests are the safety net.
