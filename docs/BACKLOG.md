...
- [ ] **Source-registry factory** `needs-design` — auto-generate fetcher + slot + UI rows from a single `SOURCE_REGISTRY` entry for the star-catalog and volume families (galaxy catalogs unified in plan 04c). → [details](backlog/2026-06-29-source-registry-factory.md)
...
- [ ] **`famous_stars_meta.json` demands unconditionally at boot** `ready` — 118 KB on every visit for a sidecar only the InfoCard reads; the `pgcAlias` row's `demand: (ctx) => ctx.ui.paletteOpen` (D6) is now the RTK-shaped precedent to follow.
...
- [ ] **Move `scaleFadeBands` + its four anchor constants to `src/data`** `ready` — the table is declared data importing thresholds from `services/`; re-home both so `data/` stays a leaf. User-ruled separate PR. → [details](backlog/2026-08-31-scale-fade-bands-to-data.md)
- [ ] **Two `FocusBoundEffect` switches swallow new kinds** `ready` — `clipFociReady` and `stubResolveClipFoci` dispatch through a silent `default:` instead of `compileClip`'s `never` guard; the `spinToId` incident that surfaced this is since patched in all three, but the silent-default asymmetry remains open.
...
- [ ] **The famous-star sidecar meta has no data-store home** `ready` — the galaxy half landed on `GalaxyStore`; give the star twin a `BodyStore` home when the star Layer needs an engine-side reader. → [details](backlog/2026-07-30-meta-getters-belong-on-the-data-stores.md)
...
- [ ] **Sky-cubemap band memory derived, not stored** `needs-design` — `allocateWhen` sees only `state` and runs before the frame's pose exists, so the lens band and GC distance are carried as last-frame memory; thread frame context into reconcile (or reorder it) so both derive. → [details](backlog/2026-09-09-sky-cubemap-band-memory-derived.md)
- [ ] **Hidden-subject caption still takes the click over a disc** `ready` — the overlay pick target folds every body caption first, including one whose subject is behind the body; split stamps by `occludeWeight`. → [details](backlog/2026-09-11-caption-pick-hidden-subject-takes-click.md)
...
- [ ] **Terrain mesh resolution 128** `needs-perf` — `SURFACE_TILE_MESH_RESOLUTION` 64 → 128 to match the 129-post height tiles; deferred from F2, perf A-B gated.
- [ ] **Scale bar measures from the datum, not the ground** `ready` — reads wrong wherever terrain sits far from `datumRadiusM` (Mars sites are km off); surfaced during the F4 Mars eye-check.
- [ ] **Mars global height z8–z9 from MOLA (supports z9.7)** `ready` — global band bakes only z3–z7 today; MOLA's native resolution supports a deeper global pyramid, a re-bake away.
...
- [ ] **`SurfaceImagerySource` carries identity twice** `ready` — `id`/`attribution` duplicate `provenance.sourceId`/`provenance.attribution`; collapse to `provenance` only on next touch, `id` read as `provenance.sourceId` at its ~4 call sites.
...
- [ ] **Star cut bakes one camera origin at prepare time, draw rebases about another** `ready` — mono-invisible since the two origins currently agree; port the spike's `originMpc`-carrying fix + regression test. → [details](backlog/2026-08-23-star-cut-origin-carrying.md)
...
- [ ] **Antique-atlas constellation figures (Flamsteed / Bode)** `deferred` — engraved figures as sky-pinned alpha PNGs; probe solved plates, grid removal and LaMa inpainting, figures still need hand clean-up. → [details](backlog/2026-09-14-antique-atlas-constellation-figures.md)
...
- [ ] **Star-picking deferred edge** `ready` — star deep link waits forever with Gaia disabled; small guard. → [details](backlog/2026-07-18-star-picking-deferred-edges.md)
...
- [ ] **MW point-cloud follow-ups** `ready` — four small knots from the T10 radar (orphaned WESL helpers, record-field offsets, tool↔app constants, pick bind-group injection). → [details](backlog/2026-07-08-mw-point-cloud-follow-ups.md)
- [ ] **Planet-rendering follow-ups** `ready` — three small knots from the final review (Saturn pole dual-source, runtime type-shape tests, uniform-size hardcode). → [details](backlog/2026-07-17-planet-rendering-follow-ups.md)
...
- [ ] **Third copy of the reduced-res viewport formula** `ready` — `Math.max(1, Math.floor(size / scale))` in `renderTargets.ts`, `scalarVolumePass.ts`, `zoneOfAvoidancePass.ts`; consolidate into `renderTargets.sizeOf(target)`.
- [ ] **Consolidate the renderer hygiene-basket duplications** `ready` — grow-on-demand instance buffer (×7+), 16-byte fade-scratch (×4), fullscreen-triangle (×5 + a second `.wesl` copy), sub-pixel cull (×3). → [details](backlog/2026-08-20-renderer-hygiene-basket.md)
...
- [ ] **mcpm-workbench: lift `Toggle.tsx`'s inline pill styling into a shared class** `ready` — `Toggle.tsx` hand-rolls its style inline; compose `pillToggle` (`src/components/SettingsPanel/pillToggle.module.css`) like `ToggleRow.module.css` already does.
