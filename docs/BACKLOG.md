...
- **Remove the SpaceMouse (6DOF puck) subsystem** — surfaced 2026-06-16. The WebHID 3Dconnexion-puck → per-frame camera-delta path is a niche input mode that threads a large cross-cutting surface for little payoff; flagged for removal. The blast radius is broad enough that it wants its own plan, not an inline drop — scope it before touching code. Removal surface (non-exhaustive): the `services/input/` SpaceMouse files (`spaceMouse.ts`, `spaceMouseReport.ts`, `spaceMouseAxes.ts`, `spaceMouseToCamera.ts`, `spaceMouseSensitivity.ts`, the `webhid.d.ts` ambient shim); `subsystems/spaceMouseSubsystem.ts` + its `camera/cameraDrivers.ts` driver wiring (check whether `CameraDriver` collapses to a single driver once it's gone); the `engine.input.spaceMouse` handle (`EngineInputHandle` / `EngineSpaceMouseHandle`) and `EngineSubsystemHandles.spaceMouse`; the per-frame `applyToCamera` call in `runFrame.ts`; the settings (`settingsTable` sensitivity row + `defaults.ts` seed); the React surface (`hooks/useSpaceMouseDevicePresence.ts`, the SettingsPanel sensitivity control + its CSS, `App.tsx` wiring, `useEngine` / `useEngineSettings`); the `@types/input/SpaceMouse*` + `@types/engine/**` SpaceMouse types; and the test fixtures that build `createSpaceMouseSubsystem` (e.g. `engineState.test.ts`). The `EngineCallbacks` connection/axes callbacks go too.
