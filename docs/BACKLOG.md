...
- [ ] **Earth tile page-table ancestor fallback is one level deep** `deferred` — only leaves are ever fetched, so a cell falls straight to the whole-globe base with no intermediate resident ancestor; fine at one baked level, not at Phase E depth. → [details](backlog/2026-07-30-earth-tile-page-table-ancestor-fallback.md)
...
- [ ] **Per-body zoom floor** `needs-design` — `clampDistance`'s global floor sits inside Earth, so scrolling in passes through the surface. → [details](backlog/2026-07-29-per-body-zoom-floor.md)
