...
- [ ] **Bloom fold-into-tonemap** `needs-design` — bloom is ~5 ms / 23% on solar-system; both mip levers measured-dead (5→3 and bloom0 1/3, spikes 2026-07-22, zero change), cost is the two full-res HDR endpoints — fuse the fold into `hdr→swap` (unify with the star-upsample fold item) or gate. → [details](backlog/2026-07-21-bloom-mip-count-perf.md)
