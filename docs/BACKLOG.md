...
- **Promote the Milky Way to a first-class `Source` (streamline its identity)** — **SPECCED + PLANNED 2026-06-15 (docs PR #315).** Spec: [`specs/2026-06-15-milky-way-first-class-source-design.md`](superpowers/specs/2026-06-15-milky-way-first-class-source-design.md). Three sequenced plans: Part 0 selection/target unification, Part 1 fade/source naming consistency, Part 2 MW selectable. Awaiting docs-PR merge, then per-plan execution. (The label half already landed earlier: PR #313 made the "You are here" label a first-class `milkyWay` label category and deleted `youAreHereSubsystem`; PR #312 added the `milkyWay` registry row, code 16.) Remaining open work captured by the three plans: collapse the `Selection`↔`FocusableTarget` mirror, `FadeId` Model A naming + `StructureCategory`→`StructureId`, and the pick-only galactic-centre dot + `MilkyWayInfo` + standard select→focus that retire the `__milky-way__` sentinel, the `App.tsx` onSelect special-case, and `focusOnMilkyWay`. The procedural-disk **renderer** stays bespoke by design. MW URL deep-linking (`#focus=milkyway`) is deferred (see the spec's out-of-scope).
