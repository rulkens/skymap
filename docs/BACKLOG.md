...
- [ ] **Rename `famousMeta` → `famousGalaxiesMeta`** `ready` — the star sidecar is `famousStarsMeta`; the galaxy one is unqualified. Symbols + the data file; 105 non-docs files; deploy order matters. → [details](backlog/2026-07-29-famous-galaxies-meta-rename.md)
- [ ] **`famous_stars_meta.json` loads twice, both unconditionally** `ready` — the slot demands it at boot (`demand: () => true`) and `useFamousStarsMeta` still fetches directly; give the slot a lazy demand predicate like `pgcAlias`'s one-shot `paletteOpened` and have the hook read the store.
...
- [ ] **`MAX_PLANETS = 24` drops the tail** `ready` — 21 seeds today; the draw caps and the pick does not. → [details](backlog/2026-07-29-planet-renderer-max-planets-cap.md)
...
- [ ] **Famous-star row has no count chip** `ready` — the seeded catalog reports no count, so its Stars row renders chip-less beside the survey rows; report it through `engineSourceCountReported` rather than special-casing the container.
- [ ] **Star-catalog row copy is singular** `ready` — the Stars panel renders `entry.label` ("Famous Star"), the Labels section renders `plural` ("Famous Stars"); render `plural` in the panel row, since singular is correct for the InfoCard source badge.
