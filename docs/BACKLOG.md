...
- **Extract the `createSettingsStore()` initial-settings literal into its own file** — surfaced 2026-06-16. `engine.ts:278-350` passes a ~72-line inline `EngineSettingsState` object literal to `createSettingsStore({...})` — every cluster's boot defaults (tier, galaxyCatalogs, tonemap, camera, bias, thumbnails, milkyWay, filaments, volumes, flow, debug, structures) spelled out inline in the middle of `createEngine`. It reads from `cb.initialTier` + the `DEFAULT_*` constants + `seedVolumeFields()` + `SOURCE_REGISTRY` + the registry-derived `Object.fromEntries` key sets. Pull it into a single-function file — `src/services/engine/settingsStore/buildInitialSettings.ts` exporting `buildInitialSettings(opts: { initialTier: Tier }): EngineSettingsState` (one function per file, filename = export) — so `createEngine` reads `createSettingsStore(buildInitialSettings({ initialTier: cb.initialTier ?? 'medium' }))`. Wins: the boot-defaults shape becomes independently testable (assert every cluster + every derived item row is seeded) without standing up the whole engine, and `createEngine` sheds 70 lines of construction noise. Mirrors how the per-field defaults already live in `data/defaults.ts`; this is the assembly step that composes them. Isolated cleanup; no behaviour change.
