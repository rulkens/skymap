...
- [ ] **Wheel notch routed by last frame's winner** `needs-design` — a notch on a follow→spin hand-off frame is dropped, one on the spin-off frame zooms a frozen base, one on a fly-to's pre-fold frame takes the other zoom, and the spin phase it zooms off is advanced twice. → [details](backlog/2026-09-10-wheel-notch-route-by-last-winner.md)
