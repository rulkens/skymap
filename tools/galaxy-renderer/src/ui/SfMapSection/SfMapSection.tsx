...
function SfMapSection({ diagnostics }: SfMapSectionProps): ReactNode {
  ...
  return (
    <CollapsibleSection
      ...
      // `dust.sfMapSeeding` rides along because this section's own control
      // writes it, even though it lives on the dust tier — see the control's
      // comment. The generator param blocks copy from their OWN nested
      // sections below, same split `HiiSection` uses for `dig`/`associations`.
      copyPayload={{
        fieldTuning: { sfMap, dust: { sfMapSeeding: dust.sfMapSeeding } },
      }}
    >
