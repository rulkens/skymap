...
/**
 * ViewSlice — the render layers plus the tool-local orbit camera (the trace
 * pass consumes camera + raymarch params via its own narrow options type, not
 * this one directly — Viewport maps between the two). Preview export is
 * `raymarch.previewPacked` below, not a fifth `layers` entry: it swaps the
 * raymarch layer's own data source rather than adding an independent draw,
 * so its ControlsPanel control lives in the Raymarch section.
 */
export type ViewSlice = {
  ...
  readonly raymarch: {
    ...
    /**
     * Composite the march as pure emission instead of front-to-back 'over'.
     * Defaults on: per-slab alpha goes opaque a few voxels in and buries the
     * interior otherwise.
     */
    readonly additive: boolean;
    /**
     * Integer 1-8, default 3 (main-app volume-row parity — renderTargets.ts's
     * `scale: 3`). 1 marches full-resolution; >1 marches into a
     * `floor(size/divisor)` offscreen target and bilinear-upsamples it in.
     * Fragment cost falls with the square of the divisor.
     */
    readonly divisor: number;
    /**
     * On demand, never per frame: true marches the packed export cube (the
     * real `packLogTraceVoxels`, same call as the `.scfd` leg) instead of the
     * live trace buffer — a structure check, not a brightness match (packed
     * values are log-transfer). `watchPreviewPackedSaga` owns every write.
     */
    readonly previewPacked: boolean;
    /**
     * The `sim.stepCount` at the moment `previewPacked`'s pack landed, or
     * `null` while nothing is packed — `previewPackedAtStep === sim.stepCount`
     * is whether the packed cube is still fresh enough to draw.
     */
    readonly previewPackedAtStep: number | null;
  };
  /**
   * The volumetric path tracer's own knobs — field names match `VolpathParams`
   * (volpathPass.ts) verbatim so a slice snapshot passes straight to `draw()`
   * with no remapping. `trimDensity`/`sampleWeight` duplicate the raymarch
   * layer's own pair rather than sharing them: both must agree on the
   * trace→density transfer, but are independently tunable here.
   */
  readonly pathTracer: {
    /**
     ...
     * `watchPaletteSaga` re-attaches the pass when it moves rather than
     * writing a uniform.
     */
    readonly paletteId: ScalarFieldPaletteId;
     ...
     * moves (`effectiveVolpathDivisor.ts`) — this field is always the user's
     * own setting, never the boosted value.
     */
    readonly divisor: number;
    /**
     * Once the progressive accumulator reaches this many samples,
     * `frameNeedsRender.ts` stops forcing a render on its own — Monte Carlo
     * noise falls as 1/sqrt(N), and 512 samples reads as converged at the
     * tool's default divisor. Excluded from `volpathKeyFor`: raising it
     * RESUMES accumulation, lowering it just goes clean, neither resets it.
     */
