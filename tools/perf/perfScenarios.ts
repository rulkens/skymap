 ...
 * both from inside the disc and from outside looking back at it, where the
 * galaxy fills the frame — in the Local Group, and zoomed out to the full
 * survey. Frame cost is a function of WHERE the camera is (how much geometry
 * projects, how much fill each pass touches), so a fixed pose per regime is
 ...
 * except `milky-way-outside` kept Earth as the orbit target while dollying
 * out, so those share the SAME look-at point (`EARTH_TARGET` — Earth's J2000
 * heliocentric position, ~1 AU off the origin) and differ only in `distance`
 * plus a little yaw/pitch framing: from Earth's surface (`8.9e-16` Mpc) out to
 * the full survey (`12372` Mpc), the ~30 scale-decades of the powers-of-ten
 * descent. From `milky-way` outward the camera is so far back that Earth's 1
 * AU offset from the Sun is far below one pixel, so the shared Earth target is
 * indistinguishable from the origin there. `milky-way-outside` targets the
 * galactic centre instead (~8 kpc off the origin, not ~1 AU): it sits ~22 kpc
 * outside the disc looking back at it, so the galaxy fills the frame — the
 * `milky-way` pose, sitting inside the disc looking at Earth, never exercises
 * that fill cost. `fovYRad` from the dump is the 60° default and not part of
 * `PerfPose`, dropped.
 */
...
// The shared look-at point for every scenario but `milky-way-outside`: Earth
// (its J2000 heliocentric position, matching
...
export const PERF_SCENARIOS: readonly PerfScenario[] = [
  ...
  },
  {
    ...
    pose: {
      target: [-0.00043726202, -0.0069827522, -0.0038794295],
