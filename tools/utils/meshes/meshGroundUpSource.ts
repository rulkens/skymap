 ...
 * `SCENE_MESH_BODIES` entry using it has an id in `SURFACE_FIXED_SITES`; a
 * key backing both a seated and a floating body has no single answer.
 */
...
const SEATED_IDS = new Set(SURFACE_FIXED_SITES.map((site) => site.id));
