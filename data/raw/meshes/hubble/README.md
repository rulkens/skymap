...
crinkled mirror, so the importer row's `materials=` sets `hbltel_1` (the aft
shroud and equipment-section panels, plus the array booms), `hbltel_4` (the
light shield and forward shell — the long thin tube) and `hbltel_2` (aperture
door, aft bulkhead, the two dishes) to metallic 1 at roughness 0.2, with a ×2
Base Color gain on `hbltel_1` because its mid-grey texture becomes a metal's
reflectance once metallic. The source has no normal maps, so all three take
`bump=0.08`: the colour texture's luminance stands in for height at 8 cm per
...
tinted mirror rather than cardboard. `hbltel_wfc_1` (the instrument box)
