 ...
 * `advanceStarFades` is the WRITE half: the Layer's `frame` hook calls it once
 * per real frame, it walks the octree once over the rig's views and steps
 * every per-node LOD fade, and returns the keep-ticking vote. `computeStarCut`
 * reads those ramps and never writes them — it emits each catalog's active
 * list at its current opacity into the CPU streams, so `frame`'s
 ...
 *      a mid-fade `advanceStarFades` returns the keep-ticking vote `frame`
...
import { starCutFor } from '../../../../../src/layers/starCatalog/render/cut/starCutFor';
...
function advance(
  ...
  return readStarCut(runtime, settings, ctx);
}
...
function makeRuntime(loaded: readonly { source: number; catalog: StarCatalog }[]): Runtime & {
  renderer: ReturnType<typeof makeRendererStub>;
} {
  return { renderer: makeRendererStub(loaded) } as unknown as Runtime & {
    renderer: ReturnType<typeof makeRendererStub>;
  };
}

function makeRendererStub(loaded: readonly { source: number; catalog: StarCatalog }[]) {
...
function makeSettings(opts: { master?: boolean; item?: boolean } = {}): StarCatalogSettings {
...
/** The `PassState` shape `starCatalogPass.draw` reads — settings only. */
...
describe('readStarCut liveness', () => {
  it('returns null when the master gate is off', () => {
    const runtime = makeRuntime([{ source: Source.GaiaStars, catalog: makeCatalog() }]);
    expect(
      readStarCut(runtime, makeSettings({ master: false }), makeCtx(camAtPc(inner))),
    ).toBeNull();
  });
  ...
  it('memoises on the ctx object so the walk runs once per frame', () => {
    ...
    const first = readStarCut(runtime, settings, ctx);
    const second = readStarCut(runtime, settings, ctx);
    // Same ctx → same cached object (never a second walk).
    expect(second).toBe(first);
    // loadedCatalogs was iterated exactly once for the frame.
    expect(runtime.renderer.loadedCatalogs).toHaveBeenCalledTimes(1);
  });
  ...
  it('forwards the source-independent shader scalars', () => {
    ...
    const settings = makeSettings();
    const ctx = makeCtx(camAtPc(inner + (outer - inner) * 0.5));
    const prep = advance(runtime, settings, ctx);
...
describe('readStarCut partition', () => {
  it('routes a leaf to the leaf stream only, with isAggregate 0 and multiplier 1', () => {
    ...
    const { leaf, aggregate } = onlySource(
      advance(runtime, makeSettings(), makeCtx(camAtPc(inner + (outer - inner) * 0.5))),
    );
  ...
  it('routes a fat leaf (level > 0, childMask 0) to the leaf stream, not aggregate', () => {
    const runtime = makeRuntime([{ source: Source.GaiaStars, catalog: makeFatLeafCatalog() }]);
    const { leaf, aggregate } = onlySource(
      advance(runtime, makeSettings(), makeCtx(camAtPc(inner + (outer - inner) * 0.5))),
    );
    expect(aggregate.count).toBe(0);
  ...
  it('routes an aggregate to the aggregate stream only, with isAggregate 1 and its subtree count', () => {
    const runtime = makeRuntime([{ source: Source.GaiaStars, catalog: makeAggregateCatalog() }]);
    const { leaf, aggregate } = onlySource(
      advance(runtime, makeSettings(), makeCtx(camAtPcVec(FAR_PC), 0)),
    );
...
describe('readStarCut per-node LOD fades', () => {
  ...
  it('fades a leaf IN over ~250 ms as it enters the cut, and out as it leaves', () => {
    ...
    const f2 = onlySource(advance(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 50)));
    ...
    const f3 = onlySource(advance(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 350)));
  ...
  it('advanceStarFades returns the keep-ticking vote while a node fade is in flight', () => {
    ...
    expect(advanceStarFades(runtime, settings, [makeCtx(camAtPcVec(FAR_PC), 0)])).toBe(false);

    // Frame 2: the cut flips → nodes mid-fade → the vote is true, so the
    // Layer's `frame` forwards a wake to `shouldKeepTicking`.
    expect(advanceStarFades(runtime, settings, [makeCtx(camAtPcVec(CLOSE_PC), 50)])).toBe(true);
  });
  ...
  it('advanceStarFades steps a ramp once per call, whatever readStarCut does around it', () => {
    ...
    advanceStarFades(runtime, settings, [makeCtx(camAtPcVec(FAR_PC), 0)]); // snap
    advanceStarFades(runtime, settings, [makeCtx(camAtPcVec(CLOSE_PC), 50)]);
    // Two reads on fresh ctxs, as the pick path issues between frames.
    readStarCut(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 50));
    readStarCut(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 50));
    advanceStarFades(runtime, settings, [makeCtx(camAtPcVec(CLOSE_PC), 100)]);
    ...
    const read = onlySource(readStarCut(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 100)));
  ...
  it('readStarCut alone never advances a ramp — two different ctx objects at the same nowMs leave opacity unchanged', () => {
    ...
    advance(runtime, settings, makeCtx(camAtPcVec(FAR_PC), 0));
    const first = onlySource(advance(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 50)));
    ...
    const second = onlySource(readStarCut(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 50)));
    expect(soleOpacity(second.leaf)).toBe(firstLeafOp);

    // A THIRD such read-only call still leaves it unchanged.
    const third = onlySource(readStarCut(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 50)));
...
describe('readStarCut stream aliasing across views (regression)', () => {
  ...
  // prime but BEFORE its draw (see FRAME_ORDER's ordering). If the
  ...
  it("a capture walk for a different viewSlot does not corrupt the main view's already-prepared streams", () => {
    ...
    const main = onlySource(advance(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 0)));
    ...
    readStarCut(runtime, settings, makeCtx(camAtPcVec(FAR_PC), 0, 1));
...
describe('readStarCut capture views (viewKind capture)', () => {
  ...
  it('a capture ctx called right after a main-view ctx yields every cut node at full opacity, not 0', () => {
    ...
    const capture = onlySource(readStarCut(runtime, settings, makeCtx(camAtPcVec(CLOSE_PC), 0, 1)));
  ...
  it("a capture call between two main-view calls does not perturb the main view's fade progression", () => {
    ...
    const controlRuntime = makeRuntime([{ source: Source.GaiaStars, catalog: controlCatalog }]);
    const controlSettings = makeSettings();
    advance(controlRuntime, controlSettings, makeCtx(camAtPcVec(FAR_PC), 0));
    const control2 = onlySource(
      advance(controlRuntime, controlSettings, makeCtx(camAtPcVec(CLOSE_PC), 50)),
    );
    const controlLeaf2 = soleOpacity(control2.leaf);
    const controlAgg2 = soleOpacity(control2.aggregate);
    const control3 = onlySource(
      advance(controlRuntime, controlSettings, makeCtx(camAtPcVec(CLOSE_PC), 350)),
    );
    ...
    const testRuntime = makeRuntime([{ source: Source.GaiaStars, catalog: testCatalog }]);
    const testSettings = makeSettings();
    advance(testRuntime, testSettings, makeCtx(camAtPcVec(FAR_PC), 0));
    readStarCut(testRuntime, testSettings, makeCtx(camAtPcVec(CLOSE_PC), 25, 1));
    const test2 = onlySource(advance(testRuntime, testSettings, makeCtx(camAtPcVec(CLOSE_PC), 50)));
    const testLeaf2 = soleOpacity(test2.leaf);
    const testAgg2 = soleOpacity(test2.aggregate);
    readStarCut(testRuntime, testSettings, makeCtx(camAtPcVec(CLOSE_PC), 200, 6));
    const test3 = onlySource(
      advance(testRuntime, testSettings, makeCtx(camAtPcVec(CLOSE_PC), 350)),
    );
...
describe('the frame star cut over several views', () => {
  ...
  it('a star node visible only in the second view is in the frame cut', () => {
    ...
    const awayRuntime = makeRuntime([{ source: Source.GaiaStars, catalog: makeCatalog() }]);
    const settings = makeSettings();
    advanceStarFades(awayRuntime, settings, [away]);
    expect(onlySource(computeStarCut(awayRuntime, settings, [away])).leaf.count).toBe(0);
    ...
    const runtime = makeRuntime([{ source: Source.GaiaStars, catalog: makeCatalog() }]);
    advanceStarFades(runtime, settings, rig);
    const both = onlySource(computeStarCut(runtime, settings, rig));
  ...
  it('two frame views in one frame advance fades once, and share the one cut via setFrameCut/starCutFor', () => {
    ...
    advanceStarFades(runtime, settings, [main, second]);
    const cut = computeStarCut(runtime, settings, [main, second]);
    // One 50 ms step, not two.
    expect(soleOpacity(onlySource(cut).leaf)).toBeCloseTo(crossfadeAt(CLOSE_PC) * (50 / 250), 6);

    // The Layer's `frame` hand-off: both views read the SAME cut through
    // `setFrameCut`/`starCutFor`, not a re-walk keyed on their own identity.
    runtime.renderer.setFrameCut(cut);
    expect(starCutFor(runtime as unknown as StarCatalogRuntime, settings, second)).toBe(cut);
    expect(starCutFor(runtime as unknown as StarCatalogRuntime, settings, main)).toBe(cut);
  });
  ...
  it('a frame view NOT in the advance list still reads the frame cut, with no second walk (the perf-cliff regression this exists to catch)', () => {
    ...
    advanceStarFades(runtime, settings, [main]);
    const advanced = computeStarCut(runtime, settings, [main]);
    runtime.renderer.setFrameCut(advanced);
    ...
    expect(starCutFor(runtime as unknown as StarCatalogRuntime, settings, second)).toBe(advanced);
    ...
    expect(runtime.renderer.loadedCatalogs).toHaveBeenCalledTimes(2);
  });
  ...
  it('a star cut prepared at origin A and drawn with a view at B rebases about A', () => {
    ...
    advanceStarFades(runtime, settings, [a]);
    const cut = computeStarCut(runtime, settings, [a]);
    ...
    const pass = starCatalogPass(runtime as unknown as StarCatalogRuntime);
    pass.draw!(null as unknown as GPURenderPassEncoder, viewAtB, a, makePassState(settings));

    const drawnVp = runtime.renderer.draw.mock.calls[0]![1].vp;
