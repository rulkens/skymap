 ...
 * wrapper over the `focusOn` composite: a focusId selection cue followed by
 * the concurrent camera move, in one seq.
 *
 * Cue kinds come from effectHelpers constructors:
 *   - `focusOn(id, over)` → `seq([focusId, all([moveTargetId, dollyToId])])`
...
import { focusOn } from '../../../src/services/engine/animation/effectHelpers';
import { focusId } from '../../../src/utils/animation/focusId';

const id = focusId('m87');

describe('flyAndFocusOnClip', () => {
  it('wraps the focusOn composite in a live-start clip', () => {
    ...
    expect(clip.timeline[0]).toEqual(focusOn(id, 5));
  });
