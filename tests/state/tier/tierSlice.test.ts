 ...
 * directly and assert the returned tier value (not a sub-field). They pin two
 * things: the boot default (`medium`) the reducer seeds from `@@INIT`, and that
...
describe('tierSlice', () => {
  it('reducer starts at medium', () => {
    expect(reducer(undefined, { type: '@@INIT' })).toBe('medium');
  });
