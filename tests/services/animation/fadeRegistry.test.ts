...
describe('createFadeRegistry', () => {
  ...
  it('opacityOf returns 1.0 for unregistered handles (fail-safe)', () => {
    const r = makeRegistry();
    const h: FadeId = { kind: 'survey', source: Source.SDSS };
  ...
  it('serialization is stable across handle equality', () => {
    const r = makeRegistry();
    const h1: FadeId = { kind: 'survey', source: Source.SDSS };
    const h2: FadeId = { kind: 'survey', source: Source.SDSS };
    r.register(h1, 0.5);
  ...
  it('different discriminator values produce different keys', () => {
    const r = makeRegistry();
    const a: FadeId = { kind: 'survey', source: Source.SDSS };
    const b: FadeId = { kind: 'survey', source: Source.Glade };
    r.register(a, 0.25);
    r.register(b, 0.75);
