/**
 * Parity guard: DOME_FACES in domeResample.wesl must equal the TS table
...
function weslSource(): string {
  const path = join(process.cwd(), 'src/services/gpu/shaders/domeResample/domeResample.wesl');
  return readFileSync(path, 'utf-8');
}

describe('domeResample.wesl DOME_FACES ↔ domeFaces.ts DOME_FACES', () => {
