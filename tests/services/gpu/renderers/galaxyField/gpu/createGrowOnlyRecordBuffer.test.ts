/**
 * The two failure modes here are invisible to the compiler and reach the screen
 * only as a WebGPU validation error or a silent leak: a write that overflows
 * the buffer, and the old buffer never destroyed. Nothing else in the suite can
 * reach `createGalaxyEngine`, so a fake device is the only place these are
 * observable.
 */
...
describe('createGrowOnlyRecordBuffer', () => {
  it('grows past capacity into a new buffer and destroys the old one', () => {
    const fake = makeFake();
