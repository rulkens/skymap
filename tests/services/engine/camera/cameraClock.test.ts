...
describe('accumulateFollowPan', () => {
  ...
  it('converts the world-space drag delta into the stored frame while engaged', () => {
    // The drag register's target is world-space in both regimes, so an engaged
    // follow-drag must store R̃⁻¹·delta — otherwise the strafe would be rotated
    // a second time on every read and run away from the cursor.
    const clock = createCameraClock();
...
describe('followPanWorld / addFollowPan', () => {
  it('round-trips a world delta through the stored frame', () => {
    // The load-bearing property: whatever `addFollowPan` is handed in world
    // space must come back out of `followPanWorld` unchanged under the SAME
    // correction. A transposed (or forgotten) conversion on either half breaks
    // this while both halves still look individually plausible.
    const clock = createCameraClock();
    addFollowPan(clock, [3, -2, 5], ROTATE_Z90);
    const world = followPanWorld(clock, ROTATE_Z90);
    expect(world[0]).toBeCloseTo(3, 12);
    expect(world[1]).toBeCloseTo(-2, 12);
