/**
 * applySwapFormat tests — the pre-bootstrap guard, the repeated-dispatch
 * no-op, and the reconfigure→repoint→rebuild call order (a pipeline baked
 * against the wrong colour-target format fails WebGPU validation, per the
 * plan's live-browser probe — see task-7-brief.md).
 */
