 ...
 *
 * `captureSettings` / `restoreSettings` / `applyEffect` mutate `state.settings`
 * directly; the only thing the bridge (`syncVisibilityFades`) does is fire fades
 ...
 * then do their deep-assign with no fade machinery to stand up. The bridge's own
