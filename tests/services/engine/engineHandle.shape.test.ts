 ...
 * exactly 5 sub-handles (camera, selection, sources, volumes, debug) plus 2
 * root-level members (destroy, assetSlots). Store writes go direct to Redux —
 * the tour and clip controls are driven by dispatching `startTour` /
 * `advanceTour` / `exitTour` and `playClip` / `stopClip` actions, not through a
 * handle namespace.
 */
