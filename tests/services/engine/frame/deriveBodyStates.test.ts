...
describe('deriveBodyStates', () => {
  ...
  it('places a surface-fixed site on its baked ground height (F4)', () => {
    ...
    // would read another test's cached map instead of exercising this one.
    const t = CONST_J2000 + 777_777;
    const states = deriveBodyStates(t);
    ...
    // loop built without needing a second call at the same instant.
    ...
    const expectedM = siteGroundRadiusM(site, mars.surface.datumRadiusM) + site.altitudeM;
