...
describe('clipPathDebugPass.enabled', () => {
  ...
  it('is true when renderer + snapshot both present', () => {
    ...
    const ctx = makeCtx();
    expect(clipPathDebugPass.enabled(state, ctx, slabViewOf(ctx, NEAR0))).toBe(true);
  });
});
...
describe('clipPathDebugPass.draw', () => {
  it('builds lines from the snapshot and forwards to setLines + draw', () => {
    ...
    const ctx = makeCtx();
    clipPathDebugPass.draw(PASS_STUB, slabViewOf(ctx, NEAR0), ctx, state);
  ...
  it('draws through a rebased vp, not the raw slab view-projection', () => {
    ...
    clipPathDebugPass.draw(PASS_STUB, view, ctx, state);
