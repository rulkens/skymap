...
/**
 * Build a minimal FrameView.  The `offscreenView` parameter
 * becomes the target table's 'volume' row view — the same value the layer
 * reads when calling `volumeUpsample.draw(pass, ctx.snapshot.renderTargets.viewOf('volume'))`.
 */
...
describe('volumeUpsamplePass.enabled', () => {
  ...
  it('stays enabled even when volumeUpsample is null (draw self-guards, not the gate)', () => {
    ...
    expect(
      volumeUpsamplePass.enabled(livenessState({ volumeUpsample: null }), makeCtx(), VIEW_STUB),
    ).toBe(true);
  });

  it('returns true when the shared liveness is non-null', () => {
    expect(volumeUpsamplePass.enabled(livenessState({}), makeCtx(), VIEW_STUB)).toBe(true);
  });
});
...
describe('volumeUpsamplePass.draw', () => {
  it("calls volumeUpsample.draw with the HDR pass and the target table's volume view", () => {
    ...
    volumeUpsamplePass.draw(PASS_STUB, VIEW_STUB, makeCtx(offscreenView), state);
