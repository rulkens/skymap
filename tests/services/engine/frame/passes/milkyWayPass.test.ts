...
describe('milkyWayPass pick vs draw', () => {
  it('keeps drawing but stops taking clicks once the camera is inside the disc', () => {
    ...
    expect(milkyWayPass.enabled(STATE, inside, VIEW_STUB)).toBe(true);
    expect(milkyWayPass.pickEnabled!(STATE, inside, VIEW_STUB)).toBe(false);

    // Framing the galaxy from outside: draw and pick agree again.
    const outside = makeCtx(0.15);
    expect(milkyWayPass.enabled(STATE, outside, VIEW_STUB)).toBe(true);
    expect(milkyWayPass.pickEnabled!(STATE, outside, VIEW_STUB)).toBe(true);
  });
  ...
  it('stays unpickable wherever it is invisible — pick is a strict subset of draw', () => {
    ...
    expect(milkyWayPass.enabled(STATE, dissolved, VIEW_STUB)).toBe(false);
    expect(milkyWayPass.pickEnabled!(STATE, dissolved, VIEW_STUB)).toBe(false);
  });
});
