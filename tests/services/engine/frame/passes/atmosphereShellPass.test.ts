...
// Length 2 in the shell's own frame, i.e. comfortably OUTSIDE the atmosphere
// top — this pass gates on that, an enclosed camera being `aerial-perspective`'s.
const MOCK_CAM_LOCAL: Vec3 = [0, 0, 2];
...
describe('atmosphereShellPass.enabled', () => {
  ...
  it('is false while the atmosphereShellRenderer handle is null, even for a bare ctx (handle short-circuits first)', () => {
    ...
    expect(atmosphereShellPass.enabled(state, CTX_STUB, makeBodyView('earth' as BodyId))).toBe(
      false,
    );
  });
});
...
describe('atmosphereShellPass.draw', () => {
  it('feeds mvp/camLocal the slab f64 vp and the pose off ctx.bodyPose, never view.vp', () => {
    ...
    atmosphereShellPass.draw(PASS_STUB, view, ctx, state);
  ...
  it('draws each row with ITS OWN ATMOSPHERE_PARAMS entry, and skips a row with no atmosphere table entry', () => {
    ...
    atmosphereShellPass.draw(PASS_STUB, makeBodyView('mars' as BodyId), ctx, state);
    atmosphereShellPass.draw(PASS_STUB, makeBodyView('moon' as BodyId), ctx, state);
