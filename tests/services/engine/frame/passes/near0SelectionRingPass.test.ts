...
describe('near0SelectionRingLayer.enabled', () => {
  ...
  it('is false for a structure row (the marker pass owns that halo)', () => {
    expect(
      near0SelectionRingPass.enabled(stateWith(STRUCTURE_ROW as SelectionRow), CTX, VIEW_STUB),
    ).toBe(false);
  });
  ...
  it('is false for a galaxy row (COSMO-tagged halo present, but not this slab)', () => {
    expect(near0SelectionRingPass.enabled(stateWith(GALAXY_ROW), CTX, VIEW_STUB)).toBe(false);
  });
});
