...
import { markerLinesPass } from '../../../../../src/services/engine/frame/passes/markerLinesPass';
...
describe('markerLinesPass.enabled', () => {
  ...
  it('is false when lineCount() is 0, true otherwise', () => {
    const ctx = makeCtx(new Set(), vi.fn(), vi.fn());
    expect(markerLinesPass.enabled(makeState(makeRenderer(0)), ctx, VIEW_STUB)).toBe(false);
    expect(markerLinesPass.enabled(makeState(makeRenderer(2)), ctx, VIEW_STUB)).toBe(true);
  });
});
...
describe('markerLinesPass.draw', () => {
  it("passes the foreground:0 colour view (not the depth view) as draw's 4th arg when the body pass ran", () => {
    ...
    markerLinesPass.draw(PASS_STUB, VIEW_STUB, ctx, makeState(renderer));
