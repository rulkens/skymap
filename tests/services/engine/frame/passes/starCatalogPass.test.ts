...
describe('starCatalogPass.enabled', () => {
  ...
  it('follows the master gate, the per-item toggle, and the crossfade band', () => {
    ...
    expect(starCatalogPass.enabled(makeState(renderer), insideCtx, VIEW_STUB)).toBe(true);

    const beyondCtx = makeCtx(camAtPc(outer + 1000));
    expect(starCatalogPass.enabled(makeState(renderer), beyondCtx, VIEW_STUB)).toBe(false);

    expect(
      starCatalogPass.enabled(makeState(renderer, { master: false }), insideCtx, VIEW_STUB),
    ).toBe(false);
    expect(
      starCatalogPass.enabled(makeState(renderer, { item: false }), insideCtx, VIEW_STUB),
    ).toBe(false);
  });
});
...
describe('starCatalogPass.draw', () => {
  it('draws the LEAF stream, handing every source the SAME rebased vp', () => {
    ...
    starCatalogPass.draw(PASS_STUB, view, makeCtx(camPos), makeState(renderer));
