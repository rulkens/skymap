...
import { fieldStarSpherePass } from '../../../../../src/services/engine/frame/passes/fieldStarSpherePass';
...
describe('fieldStarSpherePass.enabled', () => {
  it('is present for a nearby star with NO selection at all — the core un-braid', async () => {
    ...
    expect(fieldStarSpherePass.enabled(stateWith(cat), makeCtx(cam), makeNear0View(cam))).toBe(
      true,
    );
  });
  ...
  it('honours the hysteresis band: present stays present, absent stays absent', async () => {
    ...
    expect(
      fieldStarSpherePass.enabled(stateWith(catA), makeCtx(camAOn), makeNear0View(camAOn)),
    ).toBe(true);
    const camABand = camAt(posA, bandDist);
    expect(
      fieldStarSpherePass.enabled(stateWith(catA), makeCtx(camABand), makeNear0View(camABand)),
    ).toBe(true);
    ...
    expect(
      fieldStarSpherePass.enabled(stateWith(catB), makeCtx(camBBand), makeNear0View(camBBand)),
    ).toBe(false);
  });
  ...
  it('is absent (and clears its stored state) once the camera recedes past OFF', async () => {
    ...
    expect(fieldStarSpherePass.enabled(stateWith(cat), makeCtx(camOn), makeNear0View(camOn))).toBe(
      true,
    );
    const camOff = camAt(starPos, OFF_MPC * 4);
    expect(
      fieldStarSpherePass.enabled(stateWith(cat), makeCtx(camOff), makeNear0View(camOff)),
    ).toBe(false);
    ...
    expect(
      fieldStarSpherePass.enabled(stateWith(cat), makeCtx(camBand), makeNear0View(camBand)),
    ).toBe(false);
  });

  it('is absent while the catalog is not loaded (pre-bootstrap)', () => {
    expect(
      fieldStarSpherePass.enabled(stateWith(null), makeCtx([0, 0, 0]), makeNear0View([0, 0, 0])),
    ).toBe(false);
  });
});
...
describe('fieldStarSpherePass.draw', () => {
  it('draws the resolved present star (populated by enabled, read here)', async () => {
    ...
    expect(fieldStarSpherePass.enabled(state, makeCtx(cam), makeNear0View(cam))).toBe(true);
    fieldStarSpherePass.draw(PASS_STUB, makeNear0View(cam), makeCtx(cam), state);
...
describe('fieldStarSpherePass.drawPick', () => {
  it('packs the PRESENT star’s Gaia record index — the id the point pick packs', async () => {
    ...
    expect(fieldStarSpherePass.enabled(state, makeCtx(cam), makeNear0View(cam))).toBe(true);
    fieldStarSpherePass.drawPick!(PASS_STUB, makeNear0View(cam), makeCtx(cam), state);
