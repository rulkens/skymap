...
function makeState(earthRenderer: unknown, earth: EarthBody | null): EngineState {
  return {
    ...
    settings: {
      ...
      // frame — off by default, like the fixture's other debug booleans.
      debug: { showEarthLodOverlay: false },
    },
...
describe('earthLayer.draw', () => {
  ...
  it('packs the live debug.showEarthLodOverlay toggle at slot 32', () => {
    ...
    const view = makeNear0View();
    const state = makeState({ draw: drawSpy }, SEEDED_EARTH);
    (state.settings as unknown as { debug: { showEarthLodOverlay: boolean } }).debug.showEarthLodOverlay =
      true;
