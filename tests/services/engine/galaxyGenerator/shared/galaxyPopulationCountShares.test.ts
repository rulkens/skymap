/**
 * galaxyPopulationCountShares — the count shares both tiers weight their
 * populations by. `splitStarBudget`'s own "counts sum to totalStars" test
 * cannot catch a mistyped share, because the last population absorbs
 * whatever rounding (or a typo) left over; the analytic field has no such
 * absorber and would simply render a galaxy whose mixture does not add up.
 */
import { describe, expect, it } from 'vitest';
import { galaxyPopulationCountShares } from '../../../../../src/services/engine/galaxyGenerator/shared/galaxyPopulationCountShares';
