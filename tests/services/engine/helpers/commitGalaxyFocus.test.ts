// tests/services/engine/helpers/commitGalaxyFocus.test.ts
/**
 * commitGalaxyFocus — focused test for the highest-leverage invariants of
 * the shared focus-commit protocol.
 *
 * ### Why this file exists
 *
 * `commitGalaxyFocus` is the shared 3-call tail of three public-handle
...
// Imported AFTER the mock so commitGalaxyFocus picks it up.
...
/**
 * Minimal state + callbacks for a commitGalaxyFocus call.  Returns the spy
...
describe('commitGalaxyFocus', () => {
  ...
  it('fires setSelected → onFocusChange → tweenToGalaxy in that order when selection is provided', () => {
    ...
    commitGalaxyFocus(state, cb, info, { key: selectionKey, info: selectionInfo });
  ...
  it('passes selection.info=undefined through to setSelected when omitted', () => {
    ...
    commitGalaxyFocus(state, cb, info, { key: selectionKey });
