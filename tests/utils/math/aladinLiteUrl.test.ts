...
describe('aladinLiteUrl', () => {
  it('builds the canonical Aladin Lite URL', () => {
    ...
    expect(aladinLiteUrl(180.5, 12.3, 2)).toBe(
      'https://aladin.cds.unistra.fr/AladinLite/' +
        `?target=180.5%2012.3&fov=${2 / 60}&survey=CDS%2FP%2FDSS2%2Fcolor`,
    );
  });
  ...
  it('converts fovArcmin to degrees — 4 arcmin → fov=0.0666… deg', () => {
    expect(aladinLiteUrl(0, 0, 4)).toContain(`fov=${4 / 60}`);
  });
