...
import { surfaceTileBandRefineAllowed } from '../../../src/utils/surfaceTiles/surfaceTileBandRefineAllowed';
...
describe('earthTileBandRefineAllowed', () => {
  it('permits refinement inside a band while z is shallower than its max, and stops at max', () => {
    ...
    expect(surfaceTileBandRefineAllowed([band], 6, ...inside)).toBe(true);
    expect(surfaceTileBandRefineAllowed([band], 7, ...inside)).toBe(false);
  });
  ...
  it('a deep regional band only permits refinement past the world band inside its own bbox', () => {
    ...
    expect(surfaceTileBandRefineAllowed(bands, 10, ...insideRegional)).toBe(true);

    const outsideRegional = uv(0.1, 0.11, 0.1, 0.11);
    expect(surfaceTileBandRefineAllowed(bands, 10, ...outsideRegional)).toBe(false);
  });
  ...
  it('an antimeridian-split band pair is seen as covered by whichever side the query overlaps', () => {
    ...
    expect(surfaceTileBandRefineAllowed(bands, 10, ...eastOfSeam)).toBe(true);

    const westOfSeam = uv(0.01, 0.05, 0.4, 0.5);
    expect(surfaceTileBandRefineAllowed(bands, 10, ...westOfSeam)).toBe(true);

    const farFromSeam = uv(0.45, 0.49, 0.4, 0.5);
    expect(surfaceTileBandRefineAllowed(bands, 10, ...farFromSeam)).toBe(false);
  });
});
