 ...
 * that the pivot bundle reaches the clamp (so a zoom on a resting or spun pose
...
describe('zoomedPose', () => {
  ...
  it('floors at a pivoted body’s surface rather than the absolute minimum', () => {
    ...
    // the zoom it applies has to floor off that body's surface too — the bundle
    // must reach the clamp, not be dropped on the way through.
    const result = zoomedPose(makePose(EARTH_RADIUS_MPC * 4), 1e-6, EARTH_PIVOT);
  ...
  it('returns a fresh target array — not the input pose’s', () => {
    ...
    const result = zoomedPose(base, 2, NO_PIVOT);
