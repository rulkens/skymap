 ...
 *
 * Safety guarantee: each watcher's worker body calls getContext('reconcile')
 * only when an action arrives. With no action dispatched the workers never run,
 * so the missing context is never dereferenced. This test pins that guarantee.
 */
...
describe('rootSaga', () => {
  it('runs mainSaga without throwing even with no reconcile context registered', () => {
    ...
    // (Task 2.3) and engine context registration (Task 2.4).
