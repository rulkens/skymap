...
describe('Source enum — structure codes (cluster/supercluster/void)', () => {
  ...
  it('ALL_VISIBLE_MASK covers default-visible galaxy catalog sources only (no structure bits)', () => {
    ...
    // DesiDeep (bit 18) and DesiWedge (bit 19) — are deliberately CLEAR: both
    // drill geometries are specialist opt-in overlays, not part of the all-sky
    // default scene, so their bits stay off. Structure codes 5/6/7 stay clear
    // so the galaxy catalog draw loop doesn't accidentally gate on them.
