...
//
// CompactStarCard — the star hover-preview.  It resolves the constellation
...
describe('CompactStarCard', () => {
  it('shows the headline + constellation from the synchronous index', () => {
    render(createElement(CompactStarCard, { target: proxima }));
  ...
  it('renders name-only for a body absent from the index (no crash)', () => {
    const unindexed: StarInfo = { ...proxima, id: 'not-a-star', label: 'Nowhere' };
    const { container } = render(createElement(CompactStarCard, { target: unindexed }));
