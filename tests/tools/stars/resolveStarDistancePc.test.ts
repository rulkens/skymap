...
describe('resolveStarDistancePc', () => {
  it('prefers photogeo, then geo, then GCNS', () => {
    ...
    expect(resolveStarDistancePc({ rMedPhotogeo: 111, rMedGeo: 222, gcnsDistPc: 333 })).toBe(111);

    expect(resolveStarDistancePc({ rMedPhotogeo: null, rMedGeo: 222, gcnsDistPc: 333 })).toBe(222);

    expect(resolveStarDistancePc({ rMedPhotogeo: null, rMedGeo: null, gcnsDistPc: 333 })).toBe(333);
  });
