...
describe('ParamSliders', () => {
  ...
  it('disables Commit when canCommit=false', () => {
    render(<ParamSliders {...defaults} onStarnet={vi.fn()} onAlpha={vi.fn()} onCommit={vi.fn()} />);
  ...
  it('changing the gamma slider calls onAlpha with the new value', () => {
    const onAlpha = vi.fn();
    render(<ParamSliders {...defaults} onStarnet={vi.fn()} onAlpha={onAlpha} onCommit={vi.fn()} />);
