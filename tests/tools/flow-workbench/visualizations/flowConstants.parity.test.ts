 ...
 * `shaders/flowConstants.wesl` must equal the authoritative TS exports in
 * `constants.ts`. Because `?static` WESL linking does pure build-time linking
 * with NO value injection, the shader-side subset is a hand-written mirror — so
 * a test, not the compiler, is what keeps it from drifting. Mirrors the
 * runtime's `tests/data/selectionEncoding.test.ts` pattern (read the `.wesl` as
...
describe('flowConstants.wesl ↔ constants.ts parity', () => {
  it('each WESL constant matches the TS export of the same name', () => {
    ...
    for (const [name, tsValue] of cases) {
      ...
      expect(weslValue, `WESL constant ${name} is missing from flowConstants.wesl`).toBeDefined();
  ...
  it('every WESL constant has a corresponding TS export (no orphans)', () => {
    ...
    for (const name of wesl.keys()) {
      expect(known.has(name), `flowConstants.wesl declares ${name} with no asserted TS twin`).toBe(
        true,
      );
    }
  });
