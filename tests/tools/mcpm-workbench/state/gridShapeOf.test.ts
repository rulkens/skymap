 ...
 * hand-lists the four fields as a tripwire: unlike buildKey.test.ts/gridShapeKeyFor.test.ts
 * (which only check "differs when a field changes" and would stay green if a field silently
 * dropped OUT of the projection), this catches that drop directly. `paddingMpc` is deliberately
 * NOT one of the four (review-final.md minor 13): deriveGridBox always fits with padding 0, so
 * it can never move the derived box.
 */
