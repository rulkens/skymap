...
describe('pickGizmoHandle', () => {
  ...
  it('hits a translate arrow when the ray grazes its shaft midpoint', () => {
    ...
    // 0.2 tolerance (PICK_TOLERANCE_FRACTION(0.05)*half(4)). The closest point on the segment to
    // this ray is its own midpoint [2.2,2,3] (segment param t=1.2, within [0,2.4]), at
    // perpendicular distance exactly 0.1. Old point-only picking measured this ray only against
    // the tip [3.4,2,3] — distance sqrt(1.2²+0.1²)≈1.20, outside tolerance — so this ray would
    // have missed under the pre-fix code; it must hit now.
  ...
  it('does not pick past a translate arrow tip along the shaft line extension', () => {
    ...
    // off the line in y (inside the 0.2 tolerance if the line went unclamped). Clamped to the
  ...
  it('returns null for a ray through the box interior that misses every handle', () => {
    ...
    // z, leaving the same (Δx,Δy) offset) — comfortably outside the 0.2 tolerance.
  ...
  it('never returns a rotate handle when every RingHandle has radiusMpc 0', () => {
    ...
    // by the ray's free z) — outside the 0.1 tolerance (PICK_TOLERANCE_FRACTION(0.05)*half(2)).
