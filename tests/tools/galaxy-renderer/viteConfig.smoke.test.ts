/**
 * Smoke test: the Galaxy Renderer Vite config loads without throwing and
 * exposes the expected port + plugin list. Mirrors the flow-workbench smoke
 * test — it guards against a typo in the config that would make
 * `npm run galaxy-renderer` fail at import time. There is no API plugin
 * here, so (like the flow-workbench test) we do NOT assert one — but we DO
 * assert the wesl plugin, which is load-bearing: the `?static` shader
 * imports fail to resolve without it.
 */
import { describe, expect, it } from 'vitest';

describe('tools/galaxy-renderer/vite.config.ts', () => {
  it('exports a config with port 5400 and a react plugin', async () => {
    const mod = await import('../../../tools/galaxy-renderer/vite.config');
    ...
    expect(resolved.server?.port).toBe(5400);
