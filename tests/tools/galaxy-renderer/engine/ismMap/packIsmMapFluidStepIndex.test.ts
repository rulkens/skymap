/**
 * packSfMapFluidStepIndex — the two things a real bug could break: the
 * per-step row landing at the wrong strideBytes-aligned offset (same class
 * of bug `sfMapStepIndexData.test.ts` guards for the shared packer), and the
 ...
 * to `sfMapFluidEventWindow` (that function has its own test) — so this
 * test would actually fail if the two disagreed.
 */
import { describe, expect, it } from 'vitest';
import {
  packSfMapFluidStepIndex,
  SF_MAP_FLUID_STEP_INDEX_FLOATS,
} from '../../../../../tools/galaxy-renderer/src/engine/ismMap/packIsmMapFluidStepIndex';
...
describe('packSfMapFluidStepIndex', () => {
  it('places step index and the active-event window at each strideBytes-aligned row', () => {
    ...
    // Already sorted ascending by birthStep, as buildGalaxySfMapFluidEvents guarantees.
    ...
    const data = packSfMapFluidStepIndex(events, steps, impulseDuration, strideBytes);
...
describe('packSfMapFluidStepIndex ↔ sfMapFluidStep.wesl SfMapFluidStepIndex', () => {
  const struct = layoutWgslStruct(
    ...
    (type) => {
      ...
      if (!p) throw new Error(`SfMapFluidStepIndex field type ${type} has no layout entry`);
      return p;
    },
  );

  it('declares step, activeStart, activeEnd in that order at the offsets the packer writes to', () => {
    // packSfMapFluidStepIndex.ts writes fixed positions (base, base+1, base+2)
    ...
    // sfMap packer in this directory guards against.
  ...
  it('SF_MAP_FLUID_STEP_INDEX_FLOATS covers the whole struct', () => {
    expect(SF_MAP_FLUID_STEP_INDEX_FLOATS * 4).toBeGreaterThanOrEqual(struct.layout.size);
  });
});
