...
describe('buildCaptureUrl', () => {
  it('composes the cinema gate and the pinned instant', () => {
    ...
    expect(buildCaptureUrl({ base: 'http://localhost:5173', simTime, dome: false })).toBe(
  ...
  it('rejects a base carrying its own query or hash', () => {
    const simTime = new Date(Date.UTC(2026, 6, 31, 12, 0, 0));
    expect(() =>
      buildCaptureUrl({ base: 'http://localhost:5173/?cinema', simTime, dome: false }),
    ).toThrow(/--url/);
    expect(() =>
      buildCaptureUrl({ base: 'http://localhost:5173#t=x', simTime, dome: false }),
    ).toThrow(/--url/);
  });
  ...
  it('dome capture URL carries both gates', () => {
    const simTime = new Date(Date.UTC(2026, 6, 31, 12, 0, 0));
    expect(buildCaptureUrl({ base: 'http://localhost:5173', simTime, dome: true })).toBe(
      'http://localhost:5173/?cinema&dome#t=2026-07-31T12:00:00.000Z',
    );
  });
});
