/**
 * Smoke test: the Scene Workbench Vite config loads without throwing and
 * exposes the expected port + plugin list. Mirrors
 * tests/tools/mcpm-workbench/viteConfig.smoke.test.ts — it guards against a
 * typo in the config that would make `npm run scene-workbench` fail at
 * import time. The wesl plugin assertion is load-bearing: `?static` shader
 * imports do not resolve without it.
 */
import { describe, expect, it } from 'vitest';

describe('tools/scene-workbench/vite.config.ts', () => {
  it('exports a config with port 5600 and react + wesl plugins', async () => {
    const mod = await import('../../../tools/scene-workbench/vite.config');
    ...
    expect(resolved.server?.port).toBe(5600);
